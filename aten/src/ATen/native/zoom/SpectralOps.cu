...
struct HermitianSymmetryOffsetCalculator {
  ...
  HermitianSymmetryOffsetCalculator(
    ...
    using at::zoom::detail::IntDivider;
...
__global__ void _fft_conjugate_copy_kernel(
  ...
  HIP_KERNEL_LOOP_TYPE(index, numel, int64_t) {
...
void _fft_fill_with_conjugate_symmetry_zoom_(
  ...
  AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, dtype, "_fft_fill_with_conjugate_symmetry", [&] {
      using namespace zoom::detail;
     hipLaunchKernelGGL(( _fft_conjugate_copy_kernel), 
        dim3(GET_BLOCKS(numel)), dim3(HIP_NUM_THREADS), 0, c10::zoom::getCurrentZoomStream(), 
            numel,
...
REGISTER_PRIVATEUSE1_DISPATCH(fft_fill_with_conjugate_symmetry_stub, &_fft_fill_with_conjugate_symmetry_zoom_);
