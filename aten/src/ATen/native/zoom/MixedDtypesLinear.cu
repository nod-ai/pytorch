...
#if defined(_MSC_VER) || (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION < 11080)
...
#include <ATen/native/zoom/cutlass_extensions/epilogue_helpers.h>
#include <ATen/native/zoom/cutlass_extensions/gemm/kernel/default_fpA_intB_traits.h>
#include <ATen/native/zoom/cutlass_extensions/gemm/kernel/fpA_intB_gemm.h>
#include <ATen/native/zoom/cutlass_extensions/gemm/threadblock/default_mma.h>
#endif
...
mixed_dtypes_linear_cutlass(
  ...
  status = gemm_op.run(c10::zoom::getCurrentZoomStream());
  CUTLASS_STATUS_CHECK(status);
...
_mixed_dtypes_linear(const Tensor& input, const Tensor& weight,
  ...
  const auto dprops = at::zoom::getCurrentDeviceProperties();
