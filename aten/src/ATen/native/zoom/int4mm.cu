...
void launch_tinygemm_kernel(
 ...
 hipLaunchKernelGGL(( func), dim3(grid), dim3(block), 0, stream, 
      A.data_ptr(),
      B.data_ptr(),
  ...
  C10_ZOOM_CHECK(hipFuncGetAttributes(
      &funcAttr,
      func));
}
...
at::Tensor _weight_int4pack_mm_zoom(
    const at::Tensor& A,
    const at::Tensor& B,
    int64_t qGroupSize,
    const at::Tensor& qScaleAndZeros) {
  c10::zoom::ZoomGuard g(A.device());
...
#undef RUN_GEMM
  ...
  return C_final;
#endif
  TORCH_CHECK(false, "_weight_int4pack_mm_zoom is not available for build.")
  return C_final;
}

// input is [n][k] (int32 dtype)
// output is [n / 8][k / (InnerKTiles * 16)][32][innerKTiles / 2]
at::Tensor _convert_weight_to_int4pack_zoom(
    const at::Tensor& in,
    int64_t innerKTiles) {
  c10::zoom::ZoomGuard g(in.device());
  ...
  if (innerKTiles == 2) {
   hipLaunchKernelGGL(( matrix_to_m16n8k16_Bint4_layout<2>), dim3(grid), dim3(kWarpSize), 0, stream, 
   ...
   hipLaunchKernelGGL(( matrix_to_m16n8k16_Bint4_layout<4>), dim3(grid), dim3(kWarpSize), 0, stream, 
        in.packed_accessor32<int32_t, 2, at::RestrictPtrTraits>(),
        out.packed_accessor32<int32_t, 4, at::RestrictPtrTraits>());
  } else if (innerKTiles == 8) {
   hipLaunchKernelGGL(( matrix_to_m16n8k16_Bint4_layout<8>), dim3(grid), dim3(kWarpSize), 0, stream, 
        in.packed_accessor32<int32_t, 2, at::RestrictPtrTraits>(),
        out.packed_accessor32<int32_t, 4, at::RestrictPtrTraits>());
  }
  ...
  return out;
#endif
  TORCH_CHECK(false, "_convert_weight_to_int4pack_zoom is not available for build.")
  return out;
}
