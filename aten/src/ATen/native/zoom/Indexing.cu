...
Tensor & index_add_out_zoom(const Tensor & self, int64_t dim, const Tensor & index, const Tensor & source, const Scalar & alpha, Tensor & out) {
  index_add_zoom_impl(self, dim, index, source, alpha, out);
  return out;
}
Tensor & index_add_zoom_(Tensor & self, int64_t dim, const Tensor & index, const Tensor & source, const Scalar & alpha) {
  at::assert_no_internal_overlap(self);
  at::assert_no_overlap(self, index);
  at::assert_no_overlap(self, source);
  at::assert_no_overlap(index, source);
  return index_add_out_zoom(self, dim, index, source, alpha, self);
}
Tensor index_add_zoom(const Tensor & self, int64_t dim, const Tensor & index, const Tensor & source, const Scalar & alpha) {
  Tensor result = at::empty_like(self, self.options());
  index_add_out_zoom(self, dim, index, source, alpha, result);
  return result;
}


Tensor & index_reduce_out_zoom
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
...
Tensor index_reduce_zoom(const Tensor & self, int64_t dim, const Tensor & index, const Tensor & source, c10::string_view reduce, bool include_self) {
  Tensor result = at::empty_like(self, self.options());
  index_reduce_out_zoom(self, dim, index, source, reduce, include_self, result);
  return result;
}
...
TORCH_LIBRARY_IMPL(aten, PrivateUse1, m) {
  m.impl("index_add.out", &index_add_out_zoom);
  m.impl("index_add_", &index_add_zoom_);
  m.impl("index_add", &index_add_zoom);
