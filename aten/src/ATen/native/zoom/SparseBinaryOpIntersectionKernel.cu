...
struct HIPValueSelectionIntersectionKernel {
  static Tensor apply(
...
void mul_sparse_sparse_out_zoom_kernel(
    Tensor& result,
    const Tensor& x,
    const Tensor& y) {
  using HIPValueSelectionMulKernel = HIPValueSelectionIntersectionKernel<MulOp>;
  _sparse_binary_op_intersection_kernel_out<HIPKernelLauncher, HIPValueSelectionMulKernel>(
      result, x, y
  );
}

void sparse_mask_intersection_out_zoom_kernel(
    Tensor& result,
    const Tensor& x,
    const Tensor& y,
    const OptTensor& x_hash_opt = c10::nullopt) {
  using HIPValueRhsProjKernel = HIPValueSelectionIntersectionKernel<RhsProjOp>;
  _sparse_binary_op_intersection_kernel_out<HIPKernelLauncher, HIPValueRhsProjKernel>(
      result, x, y, x_hash_opt
  );
}

void sparse_mask_projection_out_zoom_kernel(
    Tensor& result,
    const Tensor& x,
    const Tensor& y,
    const OptTensor& x_hash_opt,
    bool accumulate_matches) {
  using HIPValueLhsProjKernel = HIPValueSelectionIntersectionKernel<LhsProjOp>;
  _sparse_binary_op_intersection_kernel_out<HIPKernelLauncher, HIPValueLhsProjKernel>(
...
REGISTER_PRIVATEUSE1_DISPATCH(mul_sparse_sparse_out_stub, &mul_sparse_sparse_out_zoom_kernel);
REGISTER_PRIVATEUSE1_DISPATCH(sparse_mask_intersection_out_stub, &sparse_mask_intersection_out_zoom_kernel);
REGISTER_PRIVATEUSE1_DISPATCH(sparse_mask_projection_out_stub, &sparse_mask_projection_out_zoom_kernel);
