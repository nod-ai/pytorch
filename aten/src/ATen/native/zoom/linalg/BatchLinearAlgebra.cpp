...
#include <ATen/native/zoom/linalg/MagmaUtils.h>
...
struct MagmaInitializer {
  MagmaInitializer() {
    ...
    magma_init();
#else
    ::at::zoom::detail::set_magma_init_fn([]{ magma_init(); });
#endif
  }
} initializer;
}  // namespace (anonymous)
...
void ldl_factor_magma(
  ...
  if (LD.is_complex()) {
    TORCH_CHECK(
        ...
        ", please set 'default' or 'hipsolver' backend with torch.backends.zoom.preferred_linalg_library");
  }
...
void ldl_factor_kernel(
  ...
  switch (preferred_backend) {
    case at::LinalgBackend::Cusolver:
      return ldl_factor_hipsolver(
          LD, pivots, info, upper, hermitian);
    case at::LinalgBackend::Magma:
      return ldl_factor_magma(LD, pivots, info, upper, hermitian);
    default:
    // By default use hipsolver if available and magma otherwise.
    // If hipsolver and magma 2.5.4+ are both available and hermitian=true,
...
void ldl_solve_kernel(
  ...
  if (LD.is_complex()) {
    TORCH_CHECK(
        !hermitian,
        "torch.linalg.ldl_solve: complex tensors with hermitian=True flag are not supported on Zoom.");
  }

  ldl_solve_hipsolver(LD, pivots, B, upper);
}

} // anonymous namespace

REGISTER_PRIVATEUSE1_DISPATCH(ldl_factor_stub, &ldl_factor_kernel)
REGISTER_PRIVATEUSE1_DISPATCH(ldl_solve_stub, &ldl_solve_kernel)
...
Tensor _cholesky_solve_helper_zoom_magma(const Tensor& self, const Tensor& A, bool upper) {
  int64_t info = 0;
  auto self_working_copy = cloneBatchedColumnMajor(self);
  auto A_working_copy = cloneBatchedColumnMajor(A);
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "cholesky_solve_zoom", [&]{
...
// Todo: hipsolverDn<T>potrsBatched only supports nrhs == 1 and does not have good performance.
//     Batched cholesky_solve is dispatched to magma.
Tensor _cholesky_solve_helper_zoom(const Tensor& self, const Tensor& A, bool upper) {
  return _cholesky_solve_helper_zoom_magma(self, A, upper);
}
...
static void apply_cholesky(const Tensor& self, bool upper, const Tensor& info) {
  ...
  TORCH_CHECK(
      false,
      "Calling torch.linalg.cholesky on a Zoom tensor requires compiling ",
  ...
  if (self.dim() == 2) {
    ...
    TORCH_INTERNAL_ASSERT(info.is_privateuseone());
...
void cholesky_helper_magma(const Tensor& input, bool upper, const Tensor& info) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
    input.scalar_type(), "cholesky_zoom", [&] {
...
REGISTER_PRIVATEUSE1_DISPATCH(cholesky_stub, &cholesky_kernel)
...
Tensor& cholesky_inverse_kernel_impl_magma(Tensor &result, Tensor& infos, bool upper) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(result.scalar_type(), "cholesky_inverse_out_zoom", [&]{
...
REGISTER_PRIVATEUSE1_DISPATCH(cholesky_inverse_stub, &cholesky_inverse_kernel_impl);
...
static void apply_lu_factor_batched_magma(const Tensor& input, const Tensor& pivots, const Tensor& infos, bool compute_pivots) {
  ...
  TORCH_CHECK(
      false,
      "Calling linalg.lu_factor on a Zoom tensor requires compiling ",
...
#ifdef USE_LINALG_SOLVER
  const auto lu_factor_hipsolver = [batch_size, m, n](const Tensor& input, const Tensor& pivots, const Tensor& infos, bool compute_pivots) {
    // In CUDA 10.2, lu_factor_looped_hipsolver does not finish the computations when the input
    // matrix is exactly singular. The returned pivots contain garbage. This breaks linalg.det
    // Now, batched_hipblas does not handle rectangular matrices, so we still dispatch to
    // looped_hipsolver even if m != n.
    constexpr bool looped_correct = true;
    if (m != n || (looped_correct && (batch_size == 1 || m >= 512))) {
      lu_factor_looped_hipsolver(input, pivots, infos, compute_pivots);
    } else {
      lu_factor_batched_hipblas(input, pivots, infos, compute_pivots);
    }
  };

  if (preferred_backend == at::LinalgBackend::Cusolver) {
    lu_factor_hipsolver(input, pivots, infos, compute_pivots);
  } else
...
#ifdef USE_LINALG_SOLVER
    // If magma batched is buggy, we use hipsolver
    // otherwise, lu_factor just works for square matrices, for non-square matrices magma batched is the fastest
    // otherwise (i.e. for square matrices), we choose between hipsolver and magma using a heuristic
...
REGISTER_PRIVATEUSE1_DISPATCH(lu_factor_stub, &lu_factor);
...
AT_ERROR("triangular_solve: MAGMA library not found in "
  ...
  // magmaTriangularSolve is calling hipBLAS and it prints
...
void triangular_solve_batched_magma(const Tensor& A, const Tensor& B, bool left, bool upper, TransposeType transpose, bool unitriangular) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "triangular_solve_zoom", [&]{
...
void triangular_solve_kernel(const Tensor& A, const Tensor& B, bool left, bool upper, TransposeType transpose, bool unitriangular) {
  // For batches smaller than 8 and matrix sizes larger than 64x64 hipBLAS forloop is faster than batched version
  if (batchCount(A) <= 8 && A.size(-1) >= 64) {
    triangular_solve_hipblas(A, B, left, upper, transpose, unitriangular);
  } else {
#if !AT_MAGMA_ENABLED()
    triangular_solve_batched_hipblas(A, B, left, upper, transpose, unitriangular);
#else
    // hipBLAS batched is faster than MAGMA batched up until 512x512, after that MAGMA is faster
    if (A.size(-1) <= 512) {
      triangular_solve_batched_hipblas(A, B, left, upper, transpose, unitriangular);
    } else {
...
REGISTER_PRIVATEUSE1_DISPATCH(triangular_solve_stub, &triangular_solve_kernel);
...
Tensor& orgqr_kernel_impl(Tensor& result, const Tensor& tau) {
  ...
  // See discussions in https://github.com/pytorch/pytorch/pull/51348 for comparison of hipSOLVER-MAGMA
  // and Windows failure.
  // For reference here is the MAGMA-based implementation: https://gist.github.com/IvanYashchuk/2db50002c9d3c1462ff769e6410ad983
#ifdef USE_LINALG_SOLVER
  return orgqr_helper_hipsolver(result, tau); // hipsolver
#else
  TORCH_CHECK(false, "Calling torch.orgqr on a Zoom tensor requires compiling ",
    "PyTorch with hipSOLVER. Please use PyTorch built with hipSOLVER support.");
#endif
}

REGISTER_PRIVATEUSE1_DISPATCH(orgqr_stub, &orgqr_kernel_impl);

void ormqr_kernel(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose) {
#ifdef USE_LINALG_SOLVER
  ormqr_hipsolver(input, tau, other, left, transpose);
#else
  TORCH_CHECK(false,
      "Calling torch.ormqr on a Zoom tensor requires compiling ",
      "PyTorch with hipSOLVER. Please use PyTorch built with hipSOLVER support.");
#endif
}

REGISTER_PRIVATEUSE1_DISPATCH(ormqr_stub, &ormqr_kernel);
...
static void apply_geqrf(const Tensor& input, const Tensor& tau) {
  ...
  TORCH_CHECK(
    false,
    "Calling torch.geqrf on a Zoom tensor requires compiling ",
    "PyTorch with MAGMA. Please use PyTorch built with MAGMA support.");
#else
...
void geqrf_kernel(const Tensor& input, const Tensor& tau) {
#ifdef USE_LINALG_SOLVER
  auto geqrf_hipsolver_backend = [](const Tensor& input, const Tensor& tau) {
      ...
      if (input.size(-2) <= 256 && batchCount(input) >= std::max<int64_t>(2, input.size(-2) / 16)) {
        return geqrf_batched_hipblas(input, tau);
      } else {
        return geqrf_hipsolver(input, tau);
      }
      return geqrf_batched_hipblas(input, tau);
  };
  ...
  // http://icl.cs.utk.edu/magma/forum/viewtopic.php?f=2&t=1015&p=2800&hilit=geqrf_gpu#p2800
    ...
    case at::LinalgBackend::Cusolver:
    default:
      return geqrf_hipsolver_backend(input, tau);
  }
#else
  return geqrf_magma(input, tau);
#endif
...
REGISTER_PRIVATEUSE1_DISPATCH(geqrf_stub, &geqrf_kernel);
...
static void apply_magma_eigh(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  TORCH_CHECK(
    false,
    "Calling torch.linalg.eigh/eigvalsh on a Zoom tensor requires compiling ",
    "PyTorch with MAGMA. Please use PyTorch built with MAGMA support.");
#else
...
void linalg_eigh_kernel(const Tensor& eigenvalues, const Tensor& eigenvectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  switch (preferred_backend) {
    ...
    case at::LinalgBackend::Cusolver:
    default:
      linalg_eigh_hipsolver(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
  }
#else
  linalg_eigh_magma(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
#endif
...
REGISTER_PRIVATEUSE1_DISPATCH(linalg_eigh_stub, &linalg_eigh_kernel);
...
#if !AT_MAGMA_ENABLED()
TORCH_CHECK(false, "Calling torch.linalg.eig on a Zoom tensor requires compiling PyTorch with MAGMA. "
...
void linalg_eig_kernel(Tensor& eigenvalues, Tensor& eigenvectors, Tensor& infos, const Tensor& input, bool compute_eigenvectors) {
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(input.is_privateuseone());
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(input.scalar_type(), "linalg_eig_out_zoom", [&]{
    apply_linalg_eig<scalar_t>(eigenvalues, eigenvectors, input_working_copy, infos, compute_eigenvectors);
  });
}

REGISTER_PRIVATEUSE1_DISPATCH(linalg_eig_stub, &linalg_eig_kernel);
...
void svd_magma(const Tensor& A,
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "svd_zoom", [&] {
    apply_svd_magma<scalar_t>(A_, full_matrices, compute_uv, U_, S_, Vh_, info_);
  });

  // Copy from CPU back to Zoom
...
void svd_kernel(const Tensor& A,
                ...
                const Tensor& info) {
#ifdef USE_LINALG_SOLVER
  // We always use hipSOLVER unless the user has specified they want to use MAGMA
  ...
  if (use_magma) {
    ...
    // svd_hipsolver computes V rather than Vh, so we pass a view of Vh.mT
    // and then conjugate Vh in-place
    svd_hipsolver(A, full_matrices, compute_uv, driver, U, S, compute_uv ? Vh.mT() : Vh, info);
...
REGISTER_PRIVATEUSE1_DISPATCH(svd_stub, &svd_kernel)
...
static void apply_lu_solve_looped_magma(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType transpose) {
  ...
  TORCH_CHECK(
      false,
      "Calling linalg.lu_solve on a Zoom tensor requires compiling ",
      "PyTorch with MAGMA. Please rebuild with MAGMA.");
#else
...
static void lu_solve_kernel(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType trans) {
  ...
  auto lu_solve_triangular = [n](const Tensor& LU, const Tensor& pivots, const Tensor& B, const TransposeType trans) {
    ...
    // LAPACK / hipblas / etc returns the permutation in an odd format
...
#ifdef USE_LINALG_SOLVER
  auto lu_solve_batched_hipblas_fn = [](const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType trans) {
    auto LU_ = maybe_expand_lu(B, LU);
    auto pivots_ = maybe_expand_pivots(B, pivots);
    lu_solve_batched_hipblas(*LU_, *pivots_, B, trans);
  };
#endif
  ...
  if (preferred_backend == at::LinalgBackend::Cusolver) {
    if (b <= 2 && n >= 64) {
      lu_solve_looped_hipsolver(LU, pivots, B, trans);
    } else {
      lu_solve_batched_hipblas_fn(LU, pivots, B, trans);
    }
    return;
  } else
#endif // ifdef USE_LINALG_SOLVER
  ...
  // Heuristic
  //if (n == k) {
  // if (k <= 16) batched_hipblas
  // else solve_triag
  //} else {
  //if (n <= 8) {
  // if (k >= 256 && NoTranspose) batched_magma
  // else batched_hipsolver
  //} else if (n <= 32) {
  //  b <= 2 looped_hipsolver
  //  k <= 8 batched_hipsolver
  //  solve_triag
  //} else if (n <= 64) {
  //  b <= 2 && (k <= 64 || adjoint) looped_hipsolver
  //  k <= 8 batched_hipsolver
  //  solve_triag
  //} else if (n <= 128) {
  //  if (b <= 2 && k <= 2) looped_hipsolver
  //  else if (k <= 2) batched_hipsolver
  //  else solve_triag
  //} else { // n > 128
  //  solve_triag
  //}
...
#else
  // No hipblas or hipsolver
  // lu_solve_triangular is almost always best
  lu_solve_triangular(LU, pivots, B, trans);
#endif // ifdef USE_LINALG_SOLVER
}

REGISTER_PRIVATEUSE1_DISPATCH(lu_solve_stub, &lu_solve_kernel);
...
static void apply_gels(const Tensor& a, Tensor& b, Tensor& infos) {
  ...
  TORCH_CHECK(
    m >= n,
    "torch.linalg.lstsq: only overdetermined systems (input.size(-2) >= input.size(-1)) are allowed on Zoom");
...
void lstsq_kernel(const Tensor& a, Tensor& b, Tensor& /*rank*/, Tensor& /*singular_values*/, Tensor& infos, double /*rcond*/, std::string /*driver_name*/)  {
  ...
  // this case is not supported by MAGMA or hipBLAS
  if (m < n) {
    TORCH_CHECK(
        false,
        "torch.linalg.lstsq: only overdetermined systems (input.size(-2) >= input.size(-1)) are allowed on Zoom. ",
        "Please rebuild with hipSOLVER.");
  } else { // m >= n
...
REGISTER_PRIVATEUSE1_DISPATCH(lstsq_stub, &lstsq_kernel);


#if defined(BUILD_LAZY_CUDA_LINALG) || defined(BUILD_LAZY_ZOOM_LINALG)
struct DispatchInitializer {
  DispatchInitializer() {
    zoom::detail::LinalgDispatch disp{_cholesky_solve_helper_zoom};
    zoom::detail::registerLinalgDispatch(disp);
  };
} initializer;
