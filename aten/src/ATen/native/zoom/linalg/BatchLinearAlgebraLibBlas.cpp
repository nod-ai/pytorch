...
// hipsolver APIs needed to be split into separate source files to
...
// BatchLinearAlgebraLibBlas.cpp, then any functions that used hipsolver
...
static hipblasOperation_t to_hipblasoperation(TransposeType trans) {
  switch (trans) {
...
void apply_geqrf_batched(const Tensor& input, const Tensor& tau) {
  auto batch_size = zoom_int_cast(batchCount(input), "batch_size");
  auto m = zoom_int_cast(input.size(-2), "m");
  auto n = zoom_int_cast(input.size(-1), "n");
  auto lda = std::max<int>(1, m);

  // hipBLAS batched geqrf requires input to be the device array of pointers to device single matrices
  ...
  at::zoom::blas::geqrfBatched(handle, m, n, input_ptr_array_data, lda, tau_ptr_array_data, &info, batch_size);
...
void geqrf_batched_hipblas(const Tensor& input, const Tensor& tau) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(input.scalar_type(), "geqrf_batched_zoom", [&]{
    apply_geqrf_batched<scalar_t>(input, tau);
  });
}

template <typename scalar_t>
static void apply_lu_factor_batched_hipblas(const Tensor& A, const Tensor& pivots, const Tensor& infos, bool get_pivots) {
  // This function just works with square matrices
  TORCH_INTERNAL_ASSERT(A.size(-2) == A.size(-1));

  auto batch_size = zoom_int_cast(batchCount(A), "batch_size");;
  auto n = zoom_int_cast(A.size(-2), "n");
  auto lda = zoom_int_cast(std::max<int>(1, n), "lda");
  ...
  at::zoom::blas::getrfBatched(n, a_ptr_array_data, lda, pivots_data, infos_data, batch_size);
}

void lu_factor_batched_hipblas(const Tensor& A, const Tensor& pivots, const Tensor& infos, bool get_pivots) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "lu_factor_hipblas", [&]{
    apply_lu_factor_batched_hipblas<scalar_t>(A, pivots, infos, get_pivots);
  });
}

template <typename scalar_t>
static void apply_lu_solve_batched_hipblas(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType transpose) {
  ...
  const auto trans = to_hipblasoperation(transpose);

  auto pivots_data = pivots.const_data_ptr<int>();
  auto batch_size = zoom_int_cast(batchCount(LU), "batch_size");;
  auto m = zoom_int_cast(LU.size(-2), "m");
  auto nrhs = zoom_int_cast(B.size(-1), "nrhs");
  auto lda = zoom_int_cast(std::max<int>(1, m), "lda");
  int info = 0;
  ...
  auto handle = at::zoom::getCurrentHIPBlasHandle();
  at::zoom::blas::getrsBatched(handle, trans, m, nrhs, const_cast<scalar_t**>(lu_ptr_array_data),
    lda, const_cast<int*>(pivots_data), b_ptr_array_data, lda, &info, batch_size);
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(info == 0);
}

void lu_solve_batched_hipblas(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType trans) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(LU.scalar_type(), "lu_solve_hipblas", [&]{
    apply_lu_solve_batched_hipblas<scalar_t>(LU, pivots, B, trans);
  });
}
...
static void apply_triangular_solve(const Tensor& A, const Tensor& B, bool left, bool upper, TransposeType transpose, bool unitriangular) {
  ...
  auto m = zoom_int_cast(left ? A.size(-1) : B.size(-2), "m");
  auto n = zoom_int_cast(B.size(-1), "n");
  auto lda = std::max<int>(1, zoom_int_cast(A.size(-2), "lda"));
  auto ldb = std::max<int>(1, zoom_int_cast(B.size(-2), "ldb"));
  ...
  for (decltype(batch_size) i = 0; i < batch_size; i++) {
    ...
    auto handle = at::zoom::getCurrentHIPBlasHandle();
    at::zoom::blas::trsm(handle, side, uplo, trans, diag, m, n, &alpha, A_working_ptr, lda, B_working_ptr, ldb);
  }
}
...
static void apply_triangular_solve_batched(const Tensor& A, const Tensor& B, bool left, bool upper, TransposeType transpose, bool unitriangular) {
  ...
  // hipBLAS batched trsm requires input to be the device array of pointers to device single matrices
  Tensor A_ptr_array = get_device_pointers<scalar_t>(A);
  Tensor B_ptr_array = get_device_pointers<scalar_t>(B);
  auto A_ptr_array_data = reinterpret_cast<scalar_t**>(A_ptr_array.data_ptr());
  auto B_ptr_array_data = reinterpret_cast<scalar_t**>(B_ptr_array.data_ptr());
  ...
  auto handle = at::zoom::getCurrentHIPBlasHandle();
  at::zoom::blas::trsmBatched(handle, side, uplo, trans, diag, m, n, &alpha, A_ptr_array_data, lda, B_ptr_array_data, ldb, batch_size);
}
...
inline void apply_gels_batched(const Tensor& A, Tensor& B, Tensor& infos) {
  ...
  auto m = zoom_int_cast(A.size(-2), "m");
  auto n = zoom_int_cast(A.size(-1), "n");

  auto nrhs = zoom_int_cast(B.size(-1), "nrhs");
  // hipBLAS from cuda10 and older doesn't work with nrhs == 0 (cuda11 works)
  // so we need to put this early return
  if (nrhs == 0) {
    return;
  }

  auto batch_size = zoom_int_cast(batchCount(B), "batch_size");
  auto lda = std::max<int>(1, m);
  auto ldb = std::max<int>(1, m);

  // hipBLAS's requirement
  TORCH_CHECK(
    m >= n,
    "torch.linalg.lstsq: only overdetermined systems (input.size(-2) >= input.size(-1)) are allowed on Zoom with hipBLAS backend.");

  // hipBLAS documentation says:
  ...
  // hipBLAS batched gels requires input to be the device array of pointers to device single matrices
  ...
  at::zoom::blas::gelsBatched<scalar_t>(
...
void gels_batched_hipblas(const Tensor& a, Tensor& b, Tensor& infos) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(a.scalar_type(), "gels_batched_hipblas", [&]{
