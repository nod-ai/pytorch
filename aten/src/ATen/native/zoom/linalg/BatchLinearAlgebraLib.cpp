...
#include <ATen/native/zoom/linalg/BatchLinearAlgebraLib.h>
...
static hipsolverOperation_t to_hipsolveroperation(TransposeType trans) {
  switch (trans) {
...
// Some hipBLAS and hipSOLVER batched routines require input to be a device array of pointers to device individual matrices
...
static Tensor get_device_pointers(const Tensor& input) {
  ...
  // hipblas/hipsolver interface requires 'int'
  int batch_size = zoom_int_cast(batchCount(input), "batch_size");
...
template <typename scalar_t>
void apply_ldl_factor_hipsolver(
    const Tensor& A,
    const Tensor& pivots,
    const Tensor& info,
    bool upper) {
  ...
  TORCH_CHECK(
      false,
      "Calling torch.linalg.ldl_factor on a Zoom tensor requires compiling ",
      "PyTorch with hipSOLVER. Please use PyTorch built with hipSOLVER support.");
#else
  auto batch_size = batchCount(A);
  auto n = zoom_int_cast(A.size(-2), "A.size(-2)");
  auto lda = zoom_int_cast(A.stride(-1), "A.stride(-1)");
  ...
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();

  int lwork = 0;
  at::zoom::solver::sytrf_bufferSize(handle, n, a_data, lda, &lwork);
...
template <typename scalar_t>
void apply_ldl_solve_hipsolver(
    const Tensor& A,
    const Tensor& pivots,
    const Tensor& B,
    bool upper) {
  ...
  TORCH_CHECK(
      false,
      "Calling torch.linalg.ldl_solve on a Zoom tensor requires compiling ",
      "PyTorch with hipSOLVER.");
#else
  ...
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();
  auto datatype = at::zoom::solver::get_hipsolver_datatype<scalar_t>();
  size_t worksize_device = 0;
  size_t worksize_host = 0;

  TORCH_HIPSOLVER_CHECK(hipsolverDnXsytrs_bufferSize(
      handle,
      uplo,
      n,
      nrhs,
  ...
  auto& device_allocator = *zoom::getZoomDeviceAllocator();
  ...
  for (const auto i : c10::irange(batch_size)) {
    ...
    TORCH_HIPSOLVER_CHECK(hipsolverDnXsytrs(
        handle,
        uplo,
        n,
        nrhs,
...
void ldl_factor_hipsolver(
  ...
  if (LD.is_complex()) {
    TORCH_CHECK(
        !hermitian,
        "torch.linalg.ldl_factor: complex tensors with hermitian=True flag are not supported with hipSOLVER backend. ",
        "Currently preferred backend is ",
        at::globalContext().linalgPreferredBackend(),
        ", please set 'default' or 'magma' backend with torch.backends.zoom.preferred_linalg_library");
  }
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      LD.scalar_type(), "ldl_factor_looped_hipsolver", [&] {
        apply_ldl_factor_hipsolver<scalar_t>(LD, pivots, info, upper);
      });
}
...
void ldl_solve_hipsolver(
    ...
    bool upper) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      LD.scalar_type(), "ldl_solve_looped_hipsolver", [&] {
        apply_ldl_solve_hipsolver<scalar_t>(LD, pivots, B, upper);
      });
}
...
// call hipsolver gesvd function to calculate svd
template<typename scalar_t>
inline static void apply_svd_hipsolver_gesvd(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  int n = zoom_int_cast(A.size(-1), "n");
  auto k = std::min(m, n);
  int lda = std::max<int>(1, m);
  int ldvh = std::max<int>(1, n);

  TORCH_INTERNAL_ASSERT(m >= n, "hipsolver gesvd only supports matrix with sizes m >= n");

  char job = compute_uv ? (full_matrices ? 'A' : 'S') : 'N';
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();

  int lwork = -1;
  at::zoom::solver::gesvd_buffersize<scalar_t>(handle, m, n, &lwork);
  ...
  int batchsize = calculate_all_batches ? zoom_int_cast(batchCount(A), "batch size")
                                        : batches.size();
...
// We'll copy A inside svd_hipsolver_gesvd
inline static void svd_hipsolver_gesvd(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  const Tensor& infos, bool full_matrices, bool compute_uv,
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "svd_zoom_gesvd", [&] {
    apply_svd_hipsolver_gesvd<scalar_t>(cloneBatchedColumnMajor(not_A_H ? A : A.mH()),
                                       not_A_H ? U : Vcopy,
                                       S,
                                       not_A_H ? Vcopy : U,
                                       infos,
...
// call hipsolver gesvdj function to calculate svd
template<typename scalar_t>
inline static void apply_svd_hipsolver_gesvdj(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  const Tensor& infos, bool full_matrices, bool compute_uv) {
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
  int m = zoom_int_cast(A.size(-2), "m");
  int n = zoom_int_cast(A.size(-1), "n");
  ...
  int batchsize = zoom_int_cast(batchCount(A), "batch size");
  int lda = A.stride(-1);
  ...
  // gesvdj_params controls the numerical accuracy of hipsolver gesvdj iterations on GPU
  hipsolverGesvdjInfo_t gesvdj_params;
  TORCH_HIPSOLVER_CHECK(hipsolverDnCreateGesvdjInfo(&gesvdj_params));

  // Todo: expose the following two parameters to users
  TORCH_HIPSOLVER_CHECK(hipsolverDnXgesvdjSetTolerance(gesvdj_params, std::numeric_limits<scalar_t>::epsilon()));
  TORCH_HIPSOLVER_CHECK(hipsolverDnXgesvdjSetMaxSweeps(gesvdj_params, hipsolver_gesvdj_max_sweeps));

  int lwork = -1;
  at::zoom::solver::gesvdj_buffersize<scalar_t>(
  ...
  for(int i = 0; i < batchsize; i++){
    at::zoom::solver::gesvdj<scalar_t>(
    ...
    // double residual = 0;
    // TORCH_HIPSOLVER_CHECK(hipsolverDnXgesvdjGetResidual(handle, gesvdj_params, &residual));
    // printf("gesvdj residual = %.6e\n", residual);
  }

  TORCH_HIPSOLVER_CHECK(hipsolverDnDestroyGesvdjInfo(gesvdj_params));
}

// wrapper around apply_svd_hipsolver_gesvdj that handles dtype dispatch
...
inline static void svd_hipsolver_gesvdj(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V, const Tensor& infos, bool full_matrices, bool compute_uv) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "svd_zoom_gesvdj", [&] {
    apply_svd_hipsolver_gesvdj<scalar_t>(A, U, S, V, infos, full_matrices, compute_uv);
  });
}

// call hipsolver gesvdj batched function to calculate svd
template<typename scalar_t>
inline static void apply_svd_hipsolver_gesvdjBatched(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  TORCH_HIPSOLVER_CHECK(hipsolverDnXgesvdjSetTolerance(gesvdj_params, std::numeric_limits<scalar_t>::epsilon()));
  TORCH_HIPSOLVER_CHECK(hipsolverDnXgesvdjSetMaxSweeps(gesvdj_params, hipsolver_gesvdj_max_sweeps));
  TORCH_HIPSOLVER_CHECK(hipsolverDnXgesvdjSetSortEig(gesvdj_params, 1));

  auto handle = at::zoom::getCurrentHIPSolverDnHandle();
  auto jobz = compute_uv ? HIPSOLVER_EIG_MODE_VECTOR : HIPSOLVER_EIG_MODE_NOVECTOR;
  at::zoom::solver::gesvdjBatched<scalar_t>(
...
inline static void svd_hipsolver_gesvdjBatched(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V, const Tensor& infos, bool full_matrices, bool compute_uv) {
  auto m = A.size(-2);
  auto n = A.size(-1);
  auto k = std::min(m, n);
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "svd_zoom_gesvdjBatched", [&] {
    apply_svd_hipsolver_gesvdjBatched<scalar_t>(A, U_, S, V_, infos, compute_uv);
  });
...
inline static void apply_svd_hipsolver_gesvdaStridedBatched(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  int m = zoom_int_cast(A.size(-2), "m");
  int n = zoom_int_cast(A.size(-1), "n");
  TORCH_INTERNAL_ASSERT(m >= n, "hipsolver gesvdaStridedBatched requires m >= n");
  int batchsize = zoom_int_cast(batchCount(A), "batch size");
  ...
  auto U_stride = compute_uv ? matrixStride(U) : ldu * rank;  // The strides for "empty matrices" are needed to satisfy hipsolver.
  ...
  int lwork = -1;
  at::zoom::solver::gesvdaStridedBatched_buffersize<scalar_t>(
  ...
  // hipSOLVER remark: if the user is confident on the accuracy of singular values and singular vectors,
  ...
  at::zoom::solver::gesvdaStridedBatched<scalar_t>(
    handle, jobz, rank, m, n, A_data, lda, A_stride, S_data, S_stride, U_data, ldu, U_stride, V_data, ldv, V_stride,
    reinterpret_cast<scalar_t*>(workspace.get()),
    lwork, infos.data_ptr<int>(),
    nullptr,  // hipSOLVER h_RnrmF is not calculated: reinterpret_cast<double*>(residual_frobenius_norm.get()),
    batchsize);
#endif
}

// We'll copy A inside svd_hipsolver_gesvdaStridedBatched
inline static void svd_hipsolver_gesvdaStridedBatched(
  ...
  const auto not_A_H = A.size(-2) >= A.size(-1);
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "svd_zoom_gesvdaStridedBatched", [&] {
    apply_svd_hipsolver_gesvdaStridedBatched<scalar_t>(
...
std::vector<int64_t> _check_gesvdj_convergence(const Tensor& infos, int64_t non_converging_info) {
  ...
  for(int64_t i = 0; i < infos.numel(); i++) {
    ...
    // From hipsolver doc, if info < 0, the i-th function call parameter is wrong,
    // which means pytorch implementation of hipsolver is wrong.
...
void svd_hipsolver(const Tensor& A,
  ...
  if (driver_v == "gesvd") {
    svd_hipsolver_gesvd(A, U, S, V, info, full_matrices, compute_uv);
  } else if (driver_v == "gesvdj") {
    // See the benchmarks in
    // https://github.com/pytorch/pytorch/pull/88502#issuecomment-1303860789
    // The m <= 32 && n <= 32 restrictions come from the limitations of the hipsolver backend. See the hipsolver docs
    if (m <= 32 && n <= 32) {
      svd_hipsolver_gesvdjBatched(cloneBatchedColumnMajor(A), U, S, V, info, full_matrices, compute_uv);
    } else {
      // gesvdj driver may be numerically unstable for large sized matrix
      svd_hipsolver_gesvdj(cloneBatchedColumnMajor(A), U, S, V, info, full_matrices, compute_uv);
    }
  } else if (driver_v == "gesvda") {
    // hipSOLVER: gesvdaStridedBatched is preferred for "tall skinny" (m > n) matrices
    // We do a transpose here to make it also work for (m < n) matrices.
    svd_hipsolver_gesvdaStridedBatched(A, U, S, V, info, full_matrices, compute_uv);
  } else {
    TORCH_CHECK(false, "torch.linalg.svd: unknown svd driver ", driver_v, " in svd_hipsolver computation. ", check_svd_doc);
  }
  ...
  if (driver_v != "gesvd") {
    ...
    if (!svd_non_converging_batches.empty()) {
      TORCH_WARN_ONCE("torch.linalg.svd: During SVD computation with the selected hipsolver driver, ",
      ...
      if (!driver.has_value()) {
        svd_hipsolver_gesvd(A, U, S, V, info, full_matrices, compute_uv, false, svd_non_converging_batches);
      }
    }
  }
...
// Implementation of Cholesky decomposition using looped hipsolverDn<T>potrf or hipsolverDnXpotrf (64-bit)
template<typename scalar_t>
inline static void apply_cholesky_hipsolver_potrf_looped(const Tensor& self_working_copy, bool upper, const Tensor& infos) {
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();
  ...
  hipDataType datatype = at::zoom::solver::get_hipsolver_datatype<scalar_t>();
  TORCH_HIPSOLVER_CHECK(hipsolverDnCreateParams(&params));
  at::zoom::solver::xpotrf_buffersize(handle, params, uplo, n, datatype, nullptr, lda, datatype, &worksize_device, &worksize_host);
  ...
  TORCH_HIPSOLVER_CHECK(hipsolverDnDestroyParams(params));
#else // USE_CUSOLVER_64_BIT
  int n_32 = zoom_int_cast(n, "n");
  int lda_32 = zoom_int_cast(lda, "lda");
  int lwork;
  at::zoom::solver::potrf_buffersize<scalar_t>(
...
// Implementation of Cholesky decomposition using batched hipsolverDn<T>potrfBatched
// Warning: hipsolverDn<T>potrfBatched doesn't work quite well when matrix size or batch size is zero.
// If you write your own C++ extension and use this function, make sure you do a zero numel check for the input.
template<typename scalar_t>
inline static void apply_cholesky_hipsolver_potrfBatched(const Tensor& self_working_copy, bool upper, const Tensor& infos) {
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();
  const auto uplo = upper ? HIPSOLVER_FILL_MODE_UPPER : HIPSOLVER_FILL_MODE_LOWER;
  const int n = zoom_int_cast(self_working_copy.size(-1), "n");
  const int lda = std::max<int>(1, n);

  const int batch_size = zoom_int_cast(batchCount(self_working_copy), "batch_size");

  // hipsolver batched kernels require input be "device array of device pointers"
  Tensor self_working_copy_array = get_device_pointers<scalar_t>(self_working_copy);

  at::zoom::solver::potrfBatched<scalar_t>(
    handle, uplo, n,
...
void cholesky_helper_hipsolver(const Tensor& input, bool upper, const Tensor& info) {
  ...
  if (use_hipsolver_potrf_batched_ && batchCount(input) > 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(input.scalar_type(), "cholesky_hipsolver", [&] {
      apply_cholesky_hipsolver_potrfBatched<scalar_t>(input, upper, info);
    });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(input.scalar_type(), "cholesky_hipsolver", [&] {
      apply_cholesky_hipsolver_potrf_looped<scalar_t>(input, upper, info);
    });
  }
}
...
template<typename scalar_t>
inline static void apply_cholesky_hipsolver_potrs(Tensor& self_working_copy, const Tensor& A_column_major_copy, bool upper, Tensor& infos) {
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();
  const auto uplo = upper ? HIPSOLVER_FILL_MODE_UPPER : HIPSOLVER_FILL_MODE_LOWER;
...
#else // USE_CUSOLVER_64_BIT
  int n_32 = zoom_int_cast(n, "n");
  int nrhs_32 = zoom_int_cast(nrhs, "nrhs");
  int lda_32 = zoom_int_cast(lda, "lda");
  int ldb_32 = zoom_int_cast(ldb, "ldb");

  for (int64_t i = 0; i < batch_size; i++) {
    at::zoom::solver::potrs<scalar_t>(
...
// hipsolverDn<t>potrsBatched only supports nrhs == 1
template<typename scalar_t>
inline static void apply_cholesky_hipsolver_potrsBatched(Tensor& self_working_copy, const Tensor& A_column_major_copy, bool upper, Tensor& infos) {
  auto handle = at::zoom::getCurrentHIPSolverDnHandle();
  const auto uplo = upper ? HIPSOLVER_FILL_MODE_UPPER : HIPSOLVER_FILL_MODE_LOWER;
  const int64_t n = self_working_copy.size(-2);
  const int64_t nrhs = self_working_copy.size(-1);
  ...
  at::zoom::solver::potrsBatched(
    handle, uplo,
    zoom_int_cast(n, "n"),
    zoom_int_cast(nrhs, "nrhs"),
    reinterpret_cast<scalar_t**>(A_ptr_array.data_ptr()),
    zoom_int_cast(lda, "lda"),
    reinterpret_cast<scalar_t**>(self_ptr_array.data_ptr()),
    zoom_int_cast(ldb, "ldb"),
    infos_ptr,
    zoom_int_cast(batch_size, "batch_size")
  );
}
...
Tensor _cholesky_solve_helper_zoom_hipsolver(const Tensor& self, const Tensor& A, bool upper) {
  ...
  if (batch_size > 1 && nrhs == 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "cholesky_zoom_potrs_batched", [&] {
      apply_cholesky_hipsolver_potrsBatched<scalar_t>(self_working_copy, A_column_major_copy, upper, infos);
    });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "cholesky_zoom_potrs", [&] {
      apply_cholesky_hipsolver_potrs<scalar_t>(self_working_copy, A_column_major_copy, upper, infos);
    });
  }
...
void _cholesky_inverse_hipsolver_potrs_based(Tensor& result, Tensor& infos, bool upper) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(result.scalar_type(), "cholesky_zoom_potri", [&] {
    apply_cholesky_hipsolver_potrs<scalar_t>(result, input_working_copy, upper, infos_gpu);
  });

  // Debug only: info of hipsolver potrs only check if the i-th parameter is wrong
...
Tensor& cholesky_inverse_kernel_impl_hipsolver(Tensor &result, Tensor& infos, bool upper) {
  _cholesky_inverse_hipsolver_potrs_based(result, infos, upper);
  return result;
}
...
/*
  ...
  * `n` - The number of columns of `input` to consider (actual sizes of `input` could be larger)
  ...
  For further details, please see the hipSOLVER documentation for GEQRF.
*/
template <typename scalar_t>
static void apply_geqrf(const Tensor& A, const Tensor& tau) {
  ...
  at::zoom::solver::xgeqrf_bufferSize<scalar_t>(
      at::zoom::getCurrentHIPSolverDnHandle(),
      params,
      m,
      n,
      A_data,
...
#else
  int lwork;
  int m_32 = zoom_int_cast(m, "m");
  int n_32 = zoom_int_cast(n, "n");
  int lda_32 = zoom_int_cast(lda, "lda");
  at::zoom::solver::geqrf_bufferSize<scalar_t>(
      at::zoom::getCurrentHIPSolverDnHandle(), m_32, n_32, A_data, lda_32, &lwork);
#endif // USE_CUSOLVER_64_BIT
  ...
  for (decltype(batch_size) i = 0; i < batch_size; i++) {
    ...
    at::zoom::solver::xgeqrf<scalar_t>(
        handle,
        params,
        m,
        n,
...
void geqrf_hipsolver(const Tensor& input, const Tensor& tau) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(input.scalar_type(), "geqrf_zoom", [&]{
...
/*
  ...
  * `transpose` - bool, determining whether to transpose (or conjugate transpose) Q before multiplying.
  ...
  For further details, please see the hipSOLVER documentation for ORMQR and UNMQR.
*/
template <typename scalar_t>
static void apply_ormqr(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose) {
  ...
  auto m = zoom_int_cast(other.size(-2), "m");
  auto n = zoom_int_cast(other.size(-1), "n");
  auto k = zoom_int_cast(tau.size(-1), "k");
  auto lda = std::max<int>(1, left ? m : n);
  auto ldc = std::max<int>(1, m);

  // get the optimal work size and allocate workspace tensor
  int lwork;
  at::zoom::solver::ormqr_bufferSize<scalar_t>(
    at::zoom::getCurrentHIPSolverDnHandle(), side, trans, m, n, k, input_data, lda, tau_data, other_data, ldc, &lwork);
...
void ormqr_hipsolver(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(input.scalar_type(), "orgmr_zoom", [&]{
...
/*
  ...
  * `tau` - Tensor containing the magnitudes of the elementary reflectors
  ...
  For further details, please see the hipSOLVER documentation for ORGQR and UNGQR.
*/
template <typename scalar_t>
inline static void apply_orgqr(Tensor& self, const Tensor& tau) {
  ...
  auto batchsize = zoom_int_cast(batchCount(self), "batch size");
  auto m = zoom_int_cast(self.size(-2), "m");
  auto n = zoom_int_cast(self.size(-1), "n");
  auto k = zoom_int_cast(tau.size(-1), "k");
  ...
  // hipSOLVER doesn't compute anything for this case, which is wrong
  ...
  // get the optimal work size and allocate workspace tensor
  int lwork;
  at::zoom::solver::orgqr_buffersize<scalar_t>(
    at::zoom::getCurrentHIPSolverDnHandle(), m, n, k, self_data, lda, tau_data, &lwork);
...
Tensor& orgqr_helper_hipsolver(Tensor& result, const Tensor& tau) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(result.scalar_type(), "orgqr_zoom", [&]{
...
static void apply_syevd(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  size_t worksize_host; // workspaceInBytesOnHost
  hipsolverDnParams_t params = NULL; // use default algorithm (currently it's the only option)
  at::zoom::solver::xsyevd_bufferSize<scalar_t>(
      at::zoom::getCurrentHIPSolverDnHandle(),
      params,
      jobz,
      uplo,
  ...
  int n_32 = zoom_int_cast(n, "n");
  int lda_32 = zoom_int_cast(lda, "lda");
  at::zoom::solver::syevd_bufferSize<scalar_t>(
      at::zoom::getCurrentHIPSolverDnHandle(), jobz, uplo, n_32, vectors_data, lda_32, values_data, &lwork);
#endif // USE_CUSOLVER_64_BIT
  ...
  for (decltype(batch_size) i = 0; i < batch_size; i++) {
    ...
    auto& host_allocator = *at::getCPUAllocator();
    auto work_host_data = host_allocator.allocate(worksize_host);
    at::zoom::solver::xsyevd<scalar_t>(
        handle,
        params,
        jobz,
        uplo,
...
static void apply_syevj(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  int n = zoom_int_cast(vectors.size(-1), "n");
  ...
  // hipSOLVER documentations says: "15 sweeps are good enough to converge to machine accuracy"
  ...
  TORCH_HIPSOLVER_CHECK(hipsolverDnCreateSyevjInfo(&syevj_params));

  // get the optimal work size and allocate workspace tensor
  int lwork;
  at::zoom::solver::syevj_bufferSize<scalar_t>(
      at::zoom::getCurrentHIPSolverDnHandle(), jobz, uplo, n, vectors_data, lda, values_data, &lwork, syevj_params);
  ...
  TORCH_HIPSOLVER_CHECK(hipsolverDnDestroySyevjInfo(syevj_params));
}
...
static void apply_syevj_batched(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  int n = zoom_int_cast(vectors.size(-1), "n");
  int lda = std::max<int>(1, n);
  int batch_size = zoom_int_cast(batchCount(vectors), "batch_size");
  ...
  hipsolverSyevjInfo_t syevj_params;
  TORCH_HIPSOLVER_CHECK(hipsolverDnCreateSyevjInfo(&syevj_params));
  TORCH_HIPSOLVER_CHECK(hipsolverDnXsyevjSetSortEig(syevj_params, 1));

  auto handle = at::zoom::getCurrentHIPSolverDnHandle();

  // get the optimal work size and allocate workspace tensor
  int lwork;
  at::zoom::solver::syevjBatched_bufferSize<scalar_t>(
      handle,
      jobz,
      uplo,
      n,
  ...
  auto& allocator = *zoom::getZoomDeviceAllocator();
  auto work_data = allocator.allocate(sizeof(scalar_t) * lwork);
  at::zoom::solver::syevjBatched<scalar_t>(
      handle,
      jobz,
      uplo,
      n,
...
static void linalg_eigh_hipsolver_syevd(const Tensor& eigenvalues, const Tensor& eigenvectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(eigenvectors.scalar_type(), "linalg_eigh_zoom", [&] {
    apply_syevd<scalar_t>(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
  });
}

static void linalg_eigh_hipsolver_syevj(const Tensor& eigenvalues, const Tensor& eigenvectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(eigenvectors.scalar_type(), "linalg_eigh_zoom", [&] {
    apply_syevj<scalar_t>(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
  });
}

static void linalg_eigh_hipsolver_syevj_batched(const Tensor& eigenvalues, const Tensor& eigenvectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(eigenvectors.scalar_type(), "linalg_eigh_zoom", [&] {
...
void linalg_eigh_hipsolver(const Tensor& eigenvalues, const Tensor& eigenvectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  linalg_eigh_hipsolver_syevj(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
}

// The 'apply_' word is used for templated by dtype functions that call an API routine
// underneath. Since the hipsolver API has a slightly different structure we do not prepend
...
void lu_factor_looped_hipsolver(const Tensor& self, const Tensor& pivots, const Tensor& infos, bool get_pivots) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
    ...
    [&self,
     ...
     &infos,
     get_pivots]() {
    const auto m = zoom_int_cast(self.size(-2), "m");
    const auto n = zoom_int_cast(self.size(-1), "n");
    ...
    const auto handle = at::zoom::getCurrentHIPSolverDnHandle();
    for (auto batch = decltype(batch_size){0}; batch < batch_size; ++batch) {
      at::zoom::solver::getrf<scalar_t>(
        handle, m, n,
  ...
  // Necessary because hipSOLVER uses nan for outputs that correspond to 0 in MAGMA for non-pivoted LU.
...
void lu_solve_looped_hipsolver(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType transpose) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(LU.scalar_type(), "lu_solve_hipsolver", [&] {
    const auto trans = to_hipsolveroperation(transpose);
    int n = zoom_int_cast(LU.size(-2), "n");
    int nrhs = zoom_int_cast(B.size(-1), "nrhs");
    auto batch_size = batchCount(B);
    ...
    int leading_dimension = zoom_int_cast(std::max<int>(1, n), "leading_dimension");
