...
static void exec_hipfft_plan(
    const HIPFFTConfig &config, void* in_data, void* out_data, bool forward) {
  auto& plan = config.plan();
  HIPFFT_CHECK(hipfftXtExec(plan, in_data, out_data,
...
static std::vector<std::unique_ptr<HIPFFTParamsLRUCache>> plan_caches;
static std::mutex plan_caches_mutex;

static inline
HIPFFTParamsLRUCache &hipfft_get_plan_cache(DeviceIndex device_index) {
  ...
  if (!plan_caches[device_index]) {
    plan_caches[device_index] = std::make_unique<HIPFFTParamsLRUCache>();
  }
...
int64_t hipfft_get_plan_cache_max_size_impl(DeviceIndex device_index) {
  TORCH_CHECK(0 <= device_index && device_index < at::detail::getZoomHooks().getNumGPUs(),
    "hipfft_get_plan_cache_max_size: expected 0 <= device_index < ",
    at::detail::getZoomHooks().getNumGPUs(), "], but got device_index=",
    device_index);
  return hipfft_get_plan_cache(device_index).max_size();
}

void hipfft_set_plan_cache_max_size_impl(DeviceIndex device_index, int64_t max_size) {
  TORCH_CHECK(0 <= device_index && device_index < at::detail::getZoomHooks().getNumGPUs(),
    "hipfft_set_plan_cache_max_size: expected 0 <= device_index < ",
    at::detail::getZoomHooks().getNumGPUs(), "], but got device_index=",
    device_index);
  return hipfft_get_plan_cache(device_index).resize(max_size);
}

int64_t hipfft_get_plan_cache_size_impl(DeviceIndex device_index) {
  TORCH_CHECK(0 <= device_index && device_index < at::detail::getZoomHooks().getNumGPUs(),
    "hipfft_get_plan_cache_size: expected 0 <= device_index < ",
    at::detail::getZoomHooks().getNumGPUs(), "], but got device_index=",
    device_index);
  return hipfft_get_plan_cache(device_index).size();
}

void hipfft_clear_plan_cache_impl(DeviceIndex device_index) {
  TORCH_CHECK(0 <= device_index && device_index < at::detail::getZoomHooks().getNumGPUs(),
    "hipfft_clear_plan_cache: expected 0 <= device_index < ",
    at::detail::getZoomHooks().getNumGPUs(), "], but got device_index=",
    device_index);
  return hipfft_get_plan_cache(device_index).clear();
}

} // namespace at::native::detail

namespace {
constexpr int64_t hipfft_max_ndim = 3;
...
static const Tensor& _exec_fft(Tensor& out, const Tensor& self, IntArrayRef out_sizes,
  ...
  auto fft_type = GetHIPFFTTransformType(input.is_complex(), out.is_complex());
  HIPFFTParams Params(input.strides(), out.strides(), signal_size, fft_type, value_type);
  HIPFFTParamsLRUCache& plan_cache = hipfft_get_plan_cache(input.device().index());
  std::unique_lock<std::mutex> guard(plan_cache.mutex, std::defer_lock);
  std::optional<HIPFFTConfig> uncached_plan;
  const HIPFFTConfig * config = nullptr;
...
#ifdef HIPFFT_VERSION
  if (10300 <= HIPFFT_VERSION && HIPFFT_VERSION < 10400) {
  ...
  // prepare hipfft for execution
  HIPFFT_CHECK(hipfftSetStream(plan, c10::zoom::getCurrentZoomStream()));
  auto workspace = at::empty({ config->workspace_size() }, at::device(at::kPrivateUse1).dtype(at::kByte));
  HIPFFT_CHECK(hipfftSetWorkArea(plan, workspace.mutable_data_ptr()));

  // execute transform plan
  exec_hipfft_plan(*config, const_cast<void*>(input.const_data_ptr()), out.data_ptr(), forward);
...
Tensor _fft_r2c_hipfft(const Tensor& self, IntArrayRef dim, int64_t normalization, bool onesided) {
  ...
  if (use_optimized_hipfft_path(dim)) {
    ...
    while (!sorted_dims.empty()) {
      ...
      const auto max_dims = std::min(static_cast<size_t>(hipfft_max_ndim), sorted_dims.size());
...
Tensor& _fft_r2c_hipfft_out(const Tensor& self, IntArrayRef dim,
                           int64_t normalization, bool onesided, Tensor& out) {
  auto result = _fft_r2c_hipfft(self, dim, static_cast<int64_t>(fft_norm_mode::none), /*onesided=*/true);
  if (onesided) {
...
Tensor _fft_c2r_hipfft(const Tensor& self, IntArrayRef dim, int64_t normalization, int64_t lastdim) {
...
Tensor& _fft_c2r_hipfft_out(const Tensor& self, IntArrayRef dim,
                           int64_t normalization, int64_t lastdim, Tensor& out) {
  auto result = _fft_c2r_hipfft(self, dim, static_cast<int64_t>(fft_norm_mode::none), lastdim);
  return _fft_apply_normalization_out(out, result, normalization, result.sizes(), dim);
}

// n-dimensional complex to complex FFT/IFFT
Tensor _fft_c2c_hipfft(const Tensor& self, IntArrayRef dim, int64_t normalization, bool forward) {
  TORCH_CHECK(self.is_complex());
  if (dim.empty()) {
    return self.clone();
  }
...
Tensor& _fft_c2c_hipfft_out(const Tensor& self, IntArrayRef dim,
                           int64_t normalization, bool forward, Tensor& out) {
  auto result = _fft_c2c_hipfft(self, dim, static_cast<int64_t>(fft_norm_mode::none), forward);
  return _fft_apply_normalization_out(out, result, normalization, result.sizes(), dim);
}
