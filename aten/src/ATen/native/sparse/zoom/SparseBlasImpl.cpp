...
#include <ATen/zoom/HIPSparseBlas.h>
#include <ATen/zoom/HIPSparseDescriptors.h>
#include <ATen/native/LinearAlgebraUtils.h>
#include <ATen/native/zoom/MiscUtils.h>
#include <ATen/native/sparse/SparseBlasImpl.h>
#include <ATen/native/sparse/zoom/SparseBlasImpl.h>
...
c10::MaybeOwned<Tensor> prepare_column_major_matrix_for_hipsparse(
    const Tensor& tensor) {
...
c10::MaybeOwned<Tensor> inline prepare_dense_matrix_for_hipsparse(
    const Tensor& tensor) {
#if defined(USE_ROCM)
  // CUDA < 11.0 doesn't support row-major layout, return column-major in this case
  return prepare_column_major_matrix_for_hipsparse(tensor);
#else
...
c10::MaybeOwned<Tensor> prepare_dense_matrix_for_hipsparse(
    const Tensor& tensor,
    IntArrayRef strides) {
...
void addmm_out_legacy(
  ...
  at::native::s_addmm_out_csr_sparse_dense_zoom_worker(nnz, m, n, k, result, beta, *result_, alpha, crow_indices, col_indices, values, *mat2_);
  if (!result.is_same(*result_)) {
    result.copy_(*result_);
  }
}

c10::MaybeOwned<Tensor> inline prepare_dense_vector_for_hipsparse(
    const Tensor& tensor) {
...
void inline bsrsv2_bsrsm2_may_need_to_sync() {
  ...
  // hipsparse bsrsv2 and bsrsm2 have a synchronization issue that may cause illegal memory access in cuda <= 11.6.x
  // See https://github.com/pytorch/pytorch/issues/71297
  ::c10::zoom::GetDevice::device_synchronize();
#endif
  // else: do nothing!
}
...
void block_sparse_triangular_solve_vec(
  ...
  c10::MaybeOwned<Tensor> X_ = prepare_dense_matrix_for_hipsparse(X);
  c10::MaybeOwned<Tensor> B_ = prepare_dense_matrix_for_hipsparse(B);

  auto block_size = zoom_int_cast(A.values().size(2), "block_size");
  auto nnzb = zoom_int_cast(A._nnz(), "nnzb");
  auto mb = zoom_int_cast(A.size(0), "mb") / block_size;

  auto desc = at::zoom::sparse::HipSparseMatDescriptor(upper, unitriangular);
  hipsparseOperation_t opA = transpose ? HIPSPARSE_OPERATION_TRANSPOSE
                                      : HIPSPARSE_OPERATION_NON_TRANSPOSE;

  auto info = at::zoom::sparse::HipSparseBsrsv2Info();
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      X.scalar_type(), "block_sparse_triangular_solve_vec", [&] {
        ...
        at::zoom::sparse::bsrsv2_bufferSize(
            handle,
            block_layout,
            opA,
            mb,
        ...
        at::zoom::sparse::bsrsv2_analysis(
            handle,
            block_layout,
            opA,
            mb,
...
void block_sparse_triangular_solve_mat(
  ...
  c10::MaybeOwned<Tensor> X_ = prepare_column_major_matrix_for_hipsparse(X);
  c10::MaybeOwned<Tensor> B_ = prepare_column_major_matrix_for_hipsparse(B);

  int ldb = zoom_int_cast(B_->stride(-1), "ldb");
  int ldx = zoom_int_cast(X_->stride(-1), "ldx");
  ...
  auto nnzb = zoom_int_cast(A._nnz(), "nnzb");
  auto mb = zoom_int_cast(A.size(0), "mb") / block_size;
  auto n = zoom_int_cast(B.size(-1), "n");

  auto desc = at::zoom::sparse::HipSparseMatDescriptor(upper, unitriangular);
  auto info = at::zoom::sparse::HipSparseBsrsm2Info();
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      X.scalar_type(), "block_sparse_triangular_solve_vec", [&] {
        ...
        at::zoom::sparse::bsrsm2_bufferSize(
            handle,
            block_layout,
            opA,
            opX,
        ...
        at::zoom::sparse::bsrsm2_analysis(
            handle,
            block_layout,
            opA,
            opX,
...
void block_sparse_mv(
  ...
  c10::MaybeOwned<Tensor> result_ = prepare_dense_vector_for_hipsparse(result);
  c10::MaybeOwned<Tensor> vec_ = prepare_dense_vector_for_hipsparse(vec);

  auto block_size = zoom_int_cast(mat.values().size(2), "block_size");
  auto nnzb = zoom_int_cast(mat._nnz(), "nnzb");
  auto mb = zoom_int_cast(mat.size(0), "mb") / block_size;
  auto nb = zoom_int_cast(mat.size(1), "nb") / block_size;
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      result.scalar_type(), "block_sparse_mv", [&] {
        ...
        auto handle = at::zoom::getCurrentHIPSparseHandle();
        auto desc = at::zoom::sparse::HipSparseMatDescriptor();
...
void block_sparse_mm(
  ...
  c10::MaybeOwned<Tensor> mat2_ = prepare_dense_matrix_for_hipsparse(mat2);
  ...
  c10::MaybeOwned<Tensor> result_ =
      prepare_column_major_matrix_for_hipsparse(result);
  ...
  int ldb = is_mat2_row_major ? zoom_int_cast(mat2_strides[ndim - 2], "ldb")
                              : zoom_int_cast(mat2_strides[ndim - 1], "ldb");
  int ldc = zoom_int_cast(result_strides[ndim - 1], "ldc");
  auto block_size = zoom_int_cast(mat1.values().size(2), "block_size");
  auto nnzb = zoom_int_cast(mat1._nnz(), "nnzb");
  auto mb = zoom_int_cast(mat1.size(0), "mb") / block_size;
  auto kb = zoom_int_cast(mat1.size(1), "nb") / block_size;
  auto n = zoom_int_cast(mat2.size(1), "n");
...
void spmm(
...
#else
  c10::MaybeOwned<Tensor> result_ = prepare_dense_matrix_for_hipsparse(result);
  c10::MaybeOwned<Tensor> mat2_ = prepare_dense_matrix_for_hipsparse(mat2);
  ...
  auto mat1_32 = at::native::_sparse_csr_tensor_unsafe(
      ...
      mat1.layout(),
      mat1.device());
  auto descA = at::zoom::sparse::HipSparseSpMatCsrDescriptor(mat1_32);
  auto algorithm = HIPSPARSE_MM_ALG_DEFAULT;
#else
  // TODO: update this to support COO sparse layout
  auto descA = at::zoom::sparse::HipSparseSpMatCsrDescriptor(mat1);
  auto algorithm = CUSPARSE_SPMM_CSR_ALG2;
#endif

  auto descB = at::zoom::sparse::HipSparseConstDnMatDescriptor(
      transpose_B ? mat2_->mT() : *mat2_);
  auto descC = at::zoom::sparse::HipSparseDnMatDescriptor(*result_);
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&] {
        ...
        hipDataType compute_type = at::zoom::getHIPDataType<opmath_t>();
        auto handle = at::zoom::getCurrentHIPSparseHandle();
...
void spgemm(
  ...
  // older versions of hipsparse on Windows segfault for complex128 dtype
  ...
  auto descA = at::zoom::sparse::HipSparseSpMatCsrDescriptor(A_32);
  auto descB = at::zoom::sparse::HipSparseSpMatCsrDescriptor(B_32);
  auto descC = at::zoom::sparse::HipSparseSpMatCsrDescriptor(C);

  auto spgemm_desc = at::zoom::sparse::HipSparseSpGEMMDescriptor();
  hipsparseOperation_t opA = HIPSPARSE_OPERATION_NON_TRANSPOSE;
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&] {
        auto beta_ = beta.to<scalar_t>();
        auto alpha_ = alpha.to<scalar_t>();
        auto compute_type = at::zoom::getHIPDataType<scalar_t>();
        auto handle = at::zoom::getCurrentHIPSparseHandle();

        // It's required to call workEstimation twice
        size_t buffer_size1 = 0;
        TORCH_HIPSPARSE_CHECK(hipsparseSpGEMM_workEstimation(
            handle,
            opA,
            opB,
            &alpha_,
        ...
        TORCH_HIPSPARSE_CHECK(hipsparseSpGEMM_compute(
            handle,
            opA,
            opB,
            &alpha_,
        ...
        // Resize result using nnz information from hipsparse
        ...
        TORCH_HIPSPARSE_CHECK(hipsparseSpGEMM_copy(
            handle,
            opA,
            opB,
            &alpha_,
...
void addmv_out_sparse_csr(
  ...
  // TODO: update this to support COO sparse layout
  auto descA = at::zoom::sparse::HipSparseSpMatCsrDescriptor(mat);
  auto descX = at::zoom::sparse::HipSparseDnVecDescriptor(*vec_);
  auto descY = at::zoom::sparse::HipSparseDnVecDescriptor(*result_);

  // hipsparseSpMVAlg_t was updated in cuda 11.2.1 (hipsparse 11.4.0)
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      ...
      "addmv_out_sparse_csr_zoom_impl",
      [&] {
        auto beta_ = beta.to<scalar_t>();
        auto alpha_ = alpha.to<scalar_t>();
        hipDataType compute_type = at::zoom::getHIPDataType<scalar_t>();
        auto handle = at::zoom::getCurrentHIPSparseHandle();

        size_t buffer_size;
        TORCH_HIPSPARSE_CHECK(hipsparseSpMV_bufferSize(
            handle,
            opA,
            &alpha_,
            descA.descriptor(),
        ...
        TORCH_HIPSPARSE_CHECK(hipsparseSpMV(
            handle,
            opA,
            &alpha_,
            descA.descriptor(),
...
void add_out_sparse_csr(
  ...
  IntArrayRef A_sizes = A.sizes();
  auto ndim = A.dim();
  int m = at::native::zoom_int_cast(A_sizes[ndim - 2], "m");
  int n = at::native::zoom_int_cast(A_sizes[ndim - 1], "n");
  ...
  int nnzA = at::native::zoom_int_cast(A_32._nnz(), "nnzA");
  int nnzB = at::native::zoom_int_cast(B_32._nnz(), "nnzB");
...
// Let's extract manually the nnz from the C_crow_indices
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      C.scalar_type(), "add_out_sparse_csr_zoom_impl", [&] {
        auto beta_ = beta.to<scalar_t>();
        auto alpha_ = alpha.to<scalar_t>();
        ...
        auto handle = at::zoom::getCurrentHIPSparseHandle();
        TORCH_HIPSPARSE_CHECK(hipsparseSetPointerMode(handle, HIPSPARSE_POINTER_MODE_HOST));

        size_t buffer_size;
        at::zoom::sparse::csrgeam2_bufferSizeExt<scalar_t>(
            handle,
            m,
            n,
            &alpha_,
        ...
        int nnzC = -1;
        at::zoom::sparse::csrgeam2Nnz<scalar_t>(
            handle,
            m,
            n,
            desc.descriptor(),
        ...
        at::zoom::sparse::csrgeam2<scalar_t>(
            handle,
            m,
            n,
            &alpha_,
...
void triangular_solve_out_sparse_csr(
  ...
  c10::MaybeOwned<Tensor> B_ = prepare_dense_matrix_for_hipsparse(B, X_->strides());

  // TODO: update this to support COO sparse layout
  auto descA = at::zoom::sparse::HipSparseSpMatCsrDescriptor(A);
  ...
  if (B.size(-1) == 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        X.scalar_type(), "triangular_solve_out_sparse_csr_zoom_impl", [&] {
          scalar_t alpha = 1;
          hipDataType compute_type = at::zoom::getHIPDataType<scalar_t>();
          auto handle = at::zoom::getCurrentHIPSparseHandle();
          size_t buffer_size;

          auto desc_spsv = at::zoom::sparse::HipSparseSpSVDescriptor();
          auto descB = at::zoom::sparse::HipSparseDnVecDescriptor(*B_);
          auto descX = at::zoom::sparse::HipSparseDnVecDescriptor(*X_);
          TORCH_HIPSPARSE_CHECK(hipsparseSpSV_bufferSize(
              handle,
              opA,
              &alpha,
              descA.descriptor(),
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSpSV_analysis(
              handle,
              opA,
              &alpha,
              descA.descriptor(),
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSpSV_solve(
              handle,
              opA,
              &alpha,
              descA.descriptor(),
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        X.scalar_type(), "triangular_solve_out_sparse_csr_zoom_impl", [&] {
          ...
          hipsparseOperation_t opB = HIPSPARSE_OPERATION_NON_TRANSPOSE;
          auto desc_spsm = at::zoom::sparse::HipSparseSpSMDescriptor();
          auto descB = at::zoom::sparse::HipSparseDnMatDescriptor(*B_);
          auto descX = at::zoom::sparse::HipSparseDnMatDescriptor(*X_);
          TORCH_HIPSPARSE_CHECK(hipsparseSpSM_bufferSize(
              handle,
              opA,
              opB,
              &alpha,
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSpSM_analysis(
              handle,
              opA,
              opB,
              &alpha,
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSpSM_solve(
              handle,
              opA,
              opB,
              &alpha,
...
void sampled_addmm_out_sparse_csr(
  ...
  c10::MaybeOwned<Tensor> A_ = prepare_dense_matrix_for_hipsparse(A);
  c10::MaybeOwned<Tensor> B_ = prepare_dense_matrix_for_hipsparse(B);
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      ...
      [&] {
        ...
        for (const auto i : c10::irange(batchCount(A))) {
          auto descA = at::zoom::sparse::HipSparseConstDnMatDescriptor(*A_, /*batch_offset=*/i);
          auto descB = at::zoom::sparse::HipSparseConstDnMatDescriptor(*B_, /*batch_offset=*/i);
          auto descC = at::zoom::sparse::HipSparseSpMatCsrDescriptor(C, /*batch_offset=*/i);
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSDDMM_bufferSize(
              handle,
              opA,
              opB,
              &alpha_,
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSDDMM_preprocess(
              handle,
              opA,
              opB,
              &alpha_,
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseSDDMM(
              handle,
              opA,
              opB,
              &alpha_,
