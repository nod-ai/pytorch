...
Tensor _sparse_semi_structured_apply_dense(
  ...
  c10::zoom::ZoomGuard device_guard(input.device());

  hipStream_t stream = c10::zoom::getCurrentZoomStream();
  size_t smem_bytes = 0;
  if (inputRowMajor && outputRowMajor) {
   hipLaunchKernelGGL(( sparse_semi_structured_apply_dense_k<true, true>)
        , dim3(p.getBlocksGrid()), dim3(p.getThreadsGrid()), smem_bytes, stream, p);
  } else if (!inputRowMajor && outputRowMajor) {
   hipLaunchKernelGGL(( sparse_semi_structured_apply_dense_k<false, true>)
        , dim3(p.getBlocksGrid()), dim3(p.getThreadsGrid()), smem_bytes, stream, p);
  } else {
    TORCH_CHECK(
        false,
