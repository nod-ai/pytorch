...
#if defined(USE_ZOOM) || defined(_MSC_VER) || (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION < 11080)
#else
#include <ATen/native/sparse/zoom/ComputeSparseTile.h>
#include <ATen/native/sparse/zoom/SparseSemiStructuredPack.h>
...
#if defined(USE_ZOOM) || defined(_MSC_VER) || (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION < 11080)
#else
struct MetadataHipSparseLt {
  // Format used by HipSparseLt
  ...
  MetadataHipSparseLt(at::Tensor metaN, at::Tensor metaT, int rows, int cols) {
        ...
        std::string algorithm)
{
  ...
  std::optional<c10::zoom::ZoomGuard> device_guard;
  if (!input.is_meta()) {
  ...
  auto launchKernel = [&](auto algo, std::string const& algo_name) {
    if (algo_name == algorithm) {
      ...
      size_t smem_bytes = 0;
     hipLaunchKernelGGL(( sparse_semi_structured_tile_kernel<KT>)
          , dim3(p.getBlocksGrid()),
             dim3(p.getThreadsGrid()),
             smem_bytes,
             c10::zoom::getCurrentZoomStream(), p, metadata, algo);
    }
  };
  ...
  bool use_cutlass)
{
  ...
  auto runTyped = [&](auto type)
  {
    ...
    if (use_cutlass) {
      ...
      return sparse_semi_structured_tile_typed<ElementT, MetadataHipSparseLt>(input, algo);
    }
  };
