...
#include <ATen/zoom/ZoomFunctions.h>
#include <ATen/NativeFunctions.h>
#else
#include <ATen/ops/_masked_softmax_native.h>
#include <ATen/ops/_log_softmax_privateuse1_dispatch.h>
#include <ATen/ops/_log_softmax_backward_data_privateuse1_dispatch.h>
#include <ATen/ops/_softmax_privateuse1_dispatch.h>
#include <ATen/ops/_softmax_backward_data_privateuse1_dispatch.h>
...
__global__ void zoom_sparse_coo_softmax_kernel(
...
template <typename scalar_t, bool LogSoftMax>
__global__ void zoom_sparse_coo_softmax_backward_kernel(
    int64_t* sorted_pool_indices,
    int64_t size,
    int64_t* pool_sizes,
    int64_t* pool_offsets,
...
void zoom_sparse_coo_softmax(
  ...
  */
  ...
  if (dim >= sparse_dim) {
    if (LogSoftMax) {
      auto new_values =
          privateuse1::_log_softmax(values, dim - sparse_dim + 1, false);
      out_values.set_(new_values);
    } else {
      auto new_values = privateuse1::_softmax(values, dim - sparse_dim + 1, false);
      out_values.set_(new_values);
    }
    return;
  }
  ...
  // If either nvalues or pool_size are zero, then zoom_sparse_coo_softmax_kernel
  ...
  if (nvalues > 0 && pool_size > 0) {
   hipLaunchKernelGGL(( zoom_sparse_coo_softmax_kernel<scalar_t, LogSoftMax>)
...
template <typename scalar_t, bool LogSoftMax>
void zoom_sparse_coo_softmax_backward(
    Tensor& grad_input,
    const Tensor& grad,
    const Tensor& output,
    const int64_t dim,
  ...
  */
  ...
  if (dim >= sparse_dim) {
    if (at::native::zoom_equal(out_offsets, grad_offsets) == true) {
      if (LogSoftMax) {
        auto r = privateuse1::_log_softmax_backward_data(
            grad_values, out_values, dim - sparse_dim + 1, input_dtype);
        values.set_(r);
      } else {
        auto r = privateuse1::_softmax_backward_data(grad_values, out_values, dim - sparse_dim + 1, input_dtype);
        values.set_(r);
      }
    } else {
        ...
        */
        if (j < grad_nnz && out_offsets_accessor[i] == grad_offsets_accessor[j]) {
          if (LogSoftMax) {
            ...
            auto r = privateuse1::_softmax_backward_data(
                grad_values[j], out_values[i], dim - sparse_dim, input_dtype);
            values[i].copy_(r);
          }
        }
  ...
  if (nvalues > 0 && pool_size > 0) {
   hipLaunchKernelGGL(( zoom_sparse_coo_softmax_backward_kernel<scalar_t, LogSoftMax>)
        , dim3(grid_size), dim3(block_size), 0, stream, 
            sorted_indices.data_ptr<int64_t>(),
            pool_size,
            pool_sizes.data_ptr<int64_t>(),
...
Tensor softmax_sparse_zoom(
  ...
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "softmax", [&] {
    zoom_sparse_coo_softmax<scalar_t, false>(output, input, dim);
  });
  return output;
}

Tensor log_softmax_sparse_zoom(
    const Tensor& input_,
    const int64_t dim_,
    const bool half_to_float) {
  Tensor input, output;
  ...
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "log_softmax", [&] {
    zoom_sparse_coo_softmax<scalar_t, true>(output, input, dim);
  });
  return output;
}

Tensor softmax_backward_sparse_zoom(
    const Tensor& grad_,
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.scalar_type(), "softmax_backward", [&] {
    zoom_sparse_coo_softmax_backward<scalar_t, false>(
        grad_input, grad, output, dim_, input_.scalar_type());
  });
  return grad_input;
}

Tensor log_softmax_backward_sparse_zoom(
    const Tensor& grad_,
    const Tensor& output_,
    int64_t dim_,
    const Tensor& input_) {
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.scalar_type(), "log_softmax_backward", [&] {
    zoom_sparse_coo_softmax_backward<scalar_t, true>(
        grad_input, grad, output, dim_, input_.scalar_type());
  });
  return grad_input;
}
