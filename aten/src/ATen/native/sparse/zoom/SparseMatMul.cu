...
void create_general_description_(hipsparseMatDescr_t& description_) {
  TORCH_HIPSPARSE_CHECK(hipsparseCreateMatDescr(&description_));
  TORCH_HIPSPARSE_CHECK(hipsparseSetMatType(description_, HIPSPARSE_MATRIX_TYPE_GENERAL));
  TORCH_HIPSPARSE_CHECK(hipsparseSetMatIndexBase(description_, HIPSPARSE_INDEX_BASE_ZERO));
}

// csrMatrixRef is used to have a representation of a raw CSR matrix representation
// comming from `sparse_sparse_matmul_zoom_kernel` function.
// Moreover this implements a RAII guard for a hipsparse descriptor
...
struct csrMatrixRef {
  ...
  csrMatrixRef(
      ...
      : csr_indices_{csr_indices},
        ...
        size_{size} {
    #if IS_CUSPARSE11_AVAILABLE()
      hipDataType hip_data_type = at::zoom::getHIPDataType<scalar_t>();
...
struct CusparseMatrixMultiplyOp {
  ...
  CusparseMatrixMultiplyOp() {
    static_assert(
      ...
      "hipsparseSpGEMM only supports data type of half, bfloat16, float, double and complex float, double.");
    // SpGEMM Computation
    TORCH_HIPSPARSE_CHECK(hipsparseSpGEMM_createDescr(&spgemmDesc));
  }
  ...
  csrOutput operator ()(
    ...
    //--------------------------------------------------------------------------
    // CUSPARSE APIs
    hipsparseHandle_t handle = at::zoom::getCurrentHIPSparseHandle();
    ...
    hipDataType computeType = at::zoom::getHIPDataType<scalar_t>();
...
struct CusparseMatrixMultiplyOp {
  csrOutput operator()(
      ...
      Tensor &output_indices)
  {
    TORCH_INTERNAL_ASSERT(false, "hipsparse csr sparse-sparse MM only supports data type of float and double.");
  }
};
...
template<> struct CusparseMatrixMultiplyOp<double> {
  ...
  CusparseMatrixMultiplyOp() {
    TORCH_HIPSPARSE_CHECK(hipsparseCreateCsrgemm2Info(&gemm2Info_));
  }
  ~CusparseMatrixMultiplyOp() {
  ...
  csrOutput Dgemm2(
    ...
    hipsparseHandle_t hipsparseHandle_ = at::zoom::getCurrentHIPSparseHandle();
    TORCH_HIPSPARSE_CHECK(hipsparseSetPointerMode(hipsparseHandle_, HIPSPARSE_POINTER_MODE_HOST));
    ...
    TORCH_HIPSPARSE_CHECK(hipsparseDcsrgemm2_bufferSizeExt(
        hipsparseHandle_,
        out.size(0),
        out.size(1),
        innerSize,
    ...
    TORCH_HIPSPARSE_CHECK(hipsparseXcsrgemm2Nnz(
        hipsparseHandle_,
        out.size(0),
        out.size(1),
        innerSize,
    ...
    TORCH_HIPSPARSE_CHECK(hipsparseDcsrgemm2(
        hipsparseHandle_,
        out.size(0),
        out.size(1),
        innerSize,
...
template<> struct CusparseMatrixMultiplyOp<float> {
  ...
  csrOutput Sgemm2(
    ...
    // Compute needed buffer size
    size_t new_bubber_sz;
    TORCH_HIPSPARSE_CHECK(hipsparseScsrgemm2_bufferSizeExt(
        hipsparseHandle_,
        out.size(0),
        out.size(1),
        innerSize,
    ...
    // Perform the gemm2 operation for doubles
    // out = alpha ∗ A ∗ B + beta ∗ C
    TORCH_HIPSPARSE_CHECK(hipsparseScsrgemm2(
        hipsparseHandle_,
        out.size(0),
        out.size(1),
        innerSize,
...
void sparse_sparse_matmul_zoom_kernel(
    Tensor& result,
    const Tensor& mat1,
    const Tensor& mat2) {
  ...
  static_assert(
    std::is_same<c10::Half, scalar_t>::value ||
        ...
        std::is_same<c10::complex<float>, scalar_t>::value ||
        std::is_same<c10::complex<double>, scalar_t>::value,
    "sparse_sparse_matmul_zoom_kernel only supports data type of half, bfloat16, float, double and complex float, double.");
...
Tensor sparse_sparse_matmul_zoom(const Tensor& mat1_, const Tensor& mat2_) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16, mat1_.scalar_type(), "sparse_matmul", [&] {
    sparse_sparse_matmul_zoom_kernel<scalar_t>(output, mat1_.coalesce(), mat2_.coalesce());
  });
#else
