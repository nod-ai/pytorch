...
__global__ void convert_indices_from_coo_to_csr_zoom_kernel(output_t* data_out, const input_t* data_in, const int64_t size, const int64_t numel) {
...
template <typename input_t, typename output_t>
void convert_indices_from_coo_to_csr_zoom(const Tensor& result, const Tensor& input, const int64_t size) {
  ...
  // Run (numel + 1) threads...
  int64_t THREADS = at::zoom::getCurrentDeviceProperties()->maxThreadsPerBlock;
  int64_t BLOCKS = (numel + THREADS) / THREADS;
  c10::zoom::ZoomStream stream = c10::zoom::getCurrentZoomStream();
 hipLaunchKernelGGL(( convert_indices_from_coo_to_csr_zoom_kernel), dim3(BLOCKS), dim3(THREADS), 0, stream, data_out, data_in, size, numel);
  C10_ZOOM_KERNEL_LAUNCH_CHECK();
}

template <typename input_t, typename output_t>
__global__ void convert_indices_from_csr_to_coo_zoom_kernel(output_t* data_out, const input_t* data_in, const int64_t nrows, const int64_t nnz, const int64_t nbatches) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
...
template <typename input_t, typename output_t>
void convert_indices_from_csr_to_coo_zoom(const Tensor& indices, const Tensor& crow_indices, const Tensor& col_indices, const bool transpose=false) {
  ...
  c10::zoom::ZoomStream stream = c10::zoom::getCurrentZoomStream();
 hipLaunchKernelGGL(( convert_indices_from_csr_to_coo_zoom_kernel), dim3(BLOCKS), dim3(THREADS), 0, stream, data_out, crow_indices_data_in, nrows, nnz, nbatches);
  C10_ZOOM_KERNEL_LAUNCH_CHECK();
}
...
Tensor& add_out_dense_sparse_compressed_zoom(
    Tensor& output,
    const Tensor& dense,
  ...
  TORCH_INTERNAL_ASSERT(dense.is_privateuseone());
...
Tensor& add_out_sparse_compressed_zoom(
    const Tensor& self,
    const SparseCsrTensor& other,
    const Scalar& alpha,
    SparseCsrTensor& out) {
  if (self.layout() == kStrided) {
    add_out_dense_sparse_compressed_zoom(out, self, other, alpha);
  } else if (other.layout() == kStrided) {
    add_out_dense_sparse_compressed_zoom(out, other, self, alpha);
  } else {
    TORCH_CHECK(
    ...
    sparse::impl::zoom::add_out_sparse_csr(self, other, Scalar(1), alpha, out);
  }
  return out;
}

TORCH_IMPL_FUNC(_convert_indices_from_coo_to_csr_structured_zoom) (
  const Tensor& input, const int64_t size, const bool out_int32, const Tensor& result
) {
  if (out_int32) {
    AT_DISPATCH_INTEGRAL_TYPES(input.scalar_type(), "convert_indices_from_coo_to_csr_zoom", [&] {
      convert_indices_from_coo_to_csr_zoom<scalar_t, int>(result, input, size);
    });
  } else {
    AT_DISPATCH_INTEGRAL_TYPES(input.scalar_type(), "convert_indices_from_coo_to_csr_zoom", [&] {
      convert_indices_from_coo_to_csr_zoom<scalar_t, int64_t>(result, input, size);
    });
  }
}

TORCH_IMPL_FUNC(_convert_indices_from_csr_to_coo_structured_zoom) (
  const Tensor& crow_indices, const Tensor& col_indices, const bool out_int32, const bool transpose, const Tensor& result
) {
  if (out_int32) {
    AT_DISPATCH_INTEGRAL_TYPES(crow_indices.scalar_type(), "convert_indices_from_csr_to_coo_zoom", [&] {
      convert_indices_from_csr_to_coo_zoom<scalar_t, int32_t>(result, crow_indices, col_indices, transpose);
    });
  } else {
    AT_DISPATCH_INTEGRAL_TYPES(crow_indices.scalar_type(), "convert_indices_from_csr_to_coo_zoom", [&] {
      convert_indices_from_csr_to_coo_zoom<scalar_t, int64_t>(result, crow_indices, col_indices, transpose);
    });
  }
}
...
__global__ void reduce_sparse_csr_dim0_zoom_kernel(acc_t* new_values,
...
Tensor reduce_sparse_csr_dim0_zoom_template(const Tensor& sparse, ReductionOp rop) {
  /*
  ...
  */
  ...
  // Set `is_zoom` = `true` in acc_type in CPU backend. Because the accumulate type
  ...
  c10::zoom::ZoomStream stream = c10::zoom::getCurrentZoomStream();
  AT_DISPATCH_INDEX_TYPES(col_indices.scalar_type(), "reduce_sparse_csr_dim0_zoom_indices",
                          [&]() {
                            index_t* col_indices_ptr = col_indices.data_ptr<index_t>();
                            index_t* new_col_indices_ptr = new_col_indices.data_ptr<index_t>();
                           hipLaunchKernelGGL(( reduce_sparse_csr_dim0_zoom_kernel), dim3(BLOCKS), dim3(THREADS), 0, stream, new_values_acc_ptr,
                                                                                               new_col_indices_ptr,
                                                                                               new_nnz,
                                                                                               values_ptr,
                                                                                               col_indices_ptr,
...
__global__ void reduce_crow_indices_dim1_zoom_kernel(index_t* new_crow_indices,
                                                     index_t* row_map,
...
template <typename scalar_t, typename index_t, typename ReductionOp, typename acc_t>
__global__ void reduce_sparse_csr_dim1_zoom_kernel(acc_t* new_values,
                                                   const scalar_t* values,
                                                   const index_t* crow_indices,
...
template <typename scalar_t, typename ReductionOp>
Tensor reduce_sparse_csr_dim1_zoom_template(const Tensor& sparse, ReductionOp rop) {
  /*
  ...
  */
  ...
  AT_DISPATCH_INDEX_TYPES(crow_indices.scalar_type(), "reduce_sparse_csr_dim1_zoom_indices",
                          [&]() {
                           ...
                           hipLaunchKernelGGL(( reduce_crow_indices_dim1_zoom_kernel), dim3(1), dim3(1), 0, stream, new_crow_indices_ptr,
                                                                                      row_map_ptr,
                                                                                      crow_indices_ptr,
                                                                                      nrows);
                            C10_ZOOM_KERNEL_LAUNCH_CHECK();
                            ...
                            scalar_t* values_ptr = values.data_ptr<scalar_t>();
                            acc_t* new_values_acc_ptr = new_values_acc.data_ptr<acc_t>();
                           hipLaunchKernelGGL(( reduce_sparse_csr_dim1_zoom_kernel), dim3(BLOCKS), dim3(THREADS), 0, stream, new_values_acc_ptr,
                                                                                               values_ptr,
                                                                                               crow_indices_ptr,
                                                                                               row_map_ptr,
                                                                                               nrows,
...
template <typename scalar_t, typename ReductionOp>
Tensor reduce_sparse_csr_dim01_zoom_template(const Tensor& sparse, ReductionOp rop) {
...
template <typename scalar_t, typename ReductionOp>
Tensor reduce_sparse_csr_zoom_template(const Tensor& sparse, std::vector<int64_t> dims, ReductionOp rop) {
  if (dims.size() == 1) {
    if (dims[0] == 0) {
      return reduce_sparse_csr_dim0_zoom_template<scalar_t>(sparse, rop);
    } else {
      TORCH_INTERNAL_ASSERT(dims[0] == 1);
      return reduce_sparse_csr_dim1_zoom_template<scalar_t>(sparse, rop);
    }
  } else if (dims.size() == 2) {
    TORCH_INTERNAL_ASSERT(((dims[0] == 0 && dims[1] == 1) || (dims[0] == 1 && dims[1] == 0)));
    return reduce_sparse_csr_dim01_zoom_template<scalar_t>(sparse, rop);
  }
...
template <typename scalar_t, typename ReductionOp>
Tensor reduce_sparse_csr_zoom_template(const Tensor& sparse, IntArrayRef dims_to_sum, bool keepdim, ReductionOp rop) {
  ...
  TORCH_INTERNAL_ASSERT(sparse.is_privateuseone());
  ...
  return reduce_sparse_csr_zoom_template<scalar_t>(sparse, dims, rop);
}
...
Tensor _sparse_csr_sum_zoom(const Tensor& input, IntArrayRef dims_to_sum, bool keepdim, std::optional<ScalarType> dtype) {
  ScalarType dtype_ = dtype.value_or(input.scalar_type());
  Tensor input_ = at::sparse_csr::to_type(input, dtype_);
  Tensor result;
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
      kHalf, kBFloat16, input_.scalar_type(), "_sparse_csr_sum_zoom", [&] {
      // Set `is_zoom` = `true` in acc_type in CPU backend. Because the accumulate type
      // of float should be float in current scenario. In CUDA, float is the accumulate type
      // of float, while in CPU, double is the accumulate type of float.
      using acc_t = at::acc_type<scalar_t, true>;
        result = reduce_sparse_csr_zoom_template<scalar_t>(
            input_, dims_to_sum, keepdim, ReductionAddOp<acc_t>());
      });
  return result;
}

Tensor _sparse_csr_prod_zoom(const Tensor& input, IntArrayRef dims_to_reduce, bool keepdim, std::optional<ScalarType> dtype) {
  ScalarType dtype_ = dtype.value_or(input.scalar_type());
  Tensor input_ = input.to(dtype_);
  Tensor result;
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
    kHalf, kBFloat16, input_.scalar_type(), "_sparse_csr_prod_zoom",
    [&] {
      result = reduce_sparse_csr_zoom_template<scalar_t>(input_, dims_to_reduce, keepdim, ReductionMulOp<scalar_t>());
    });
  return result;
}
