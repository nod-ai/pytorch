...
#include <ATen/native/sparse/zoom/SparseHIPTensorMath.cuh>
...
#include <ATen/native/sparse/zoom/SparseBlasLegacy.h>
#include <ATen/native/sparse/zoom/SparseHIPApplyUtils.cuh>
...
#define I_INFO(tensor) zoom::detail::getTensorInfo<int64_t, uint64_t>(tensor)
#define V_INFO(tensor) zoom::detail::getTensorInfo<scalar_t, uint64_t>(tensor)
...
namespace {
  Tensor _to_csr_int(const Tensor& rowIndices, int64_t dim, int64_t nnz) {
    Tensor csr = at::empty({dim+1}, at::device(at::kPrivateUse1).dtype(kInt));
    Tensor rowIndicesInt = at::empty({rowIndices.size(0)}, at::device(at::kPrivateUse1).dtype(kInt));
    rowIndicesInt.copy_(rowIndices);
    sparse::zoom::Xcoo2csr(rowIndicesInt.data_ptr<int32_t>(), nnz, dim, csr.data_ptr<int32_t>());
    return csr;
  }
}
...
void s_addmm_out_sparse_dense_zoom_worker(int64_t nnz, int64_t m, int64_t n, int64_t k, Tensor& r_, const Scalar& beta, const Tensor& t, const Scalar& alpha, Tensor& indices, Tensor& values, const Tensor& dense) {
  ...
  s_addmm_out_csr_sparse_dense_zoom_worker(nnz, m, n, k, r_, beta, t, alpha, crow_indices, col_indices, values, dense);
}
...
Tensor& s_addmm_out_sparse_dense_zoom(Tensor& r_, const Tensor& t, const SparseTensor& sparse_, const Tensor& dense, const Scalar& beta, const Scalar& alpha) {
  TORCH_CHECK(t.is_privateuseone(), "Expected all tensors to be on the same device. addmm: expected 'self' to be Zoom, but got CPU");
  TORCH_CHECK(r_.is_privateuseone(), "Expected all tensors to be on the same device. addmm: expected 'out' to be Zoom, but got CPU");
  TORCH_CHECK(sparse_.is_privateuseone(), "Expected all tensors to be on the same device. addmm: expected 'mat1' to be Zoom, but got CPU");
  TORCH_CHECK(dense.is_privateuseone(), "Expected all tensors to be on the same device. addmm: expected 'mat2' to be Zoom, but got CPU");

  TORCH_CHECK(zoom::check_device({sparse_, r_, t, dense}));
  ...
  s_addmm_out_sparse_dense_zoom_worker(nnz, m, n, k, r_, beta, t, alpha, indices, values, dense);
  return r_;
}

Tensor& addmm_out_sparse_dense_zoom(
    const Tensor& self,
  ...
  return s_addmm_out_sparse_dense_zoom(result, *b_self, mat1, mat2, beta, alpha);
}

Tensor s_addmm_sparse_dense_zoom(
    const Tensor& t,
  ...
  s_addmm_out_sparse_dense_zoom(r, t, sparse, dense, beta, alpha);
  return r;
}

Tensor addmm_sparse_dense_zoom(
    const Tensor& self,
    const SparseTensor& mat1,
    const Tensor& mat2,
    const Scalar& beta,
  ...
  c10::MaybeOwned<Tensor> b_self = expand_size(self, {mat1.size(0), mat2.size(1)}, "addmm_out");
  return s_addmm_sparse_dense_zoom(*b_self, mat1, mat2, beta, alpha);
}

Tensor& s_addmm_sparse_dense_zoom_(
    Tensor& t,
    const SparseTensor& sparse,
    const Tensor& dense,
    const Scalar& beta,
  ...
  return s_addmm_out_sparse_dense_zoom(t, t, sparse, dense, beta, alpha);
}
...
SparseTensor& hspmm_out_sparse_zoom(
    const SparseTensor& sparse_,
    const Tensor& dense,
    SparseTensor& r_
    /* , const Scalar& alpha */) {
  TORCH_CHECK(sparse_.is_privateuseone(), "hspmm: expected 'self' to be Zoom, but got CPU");
  TORCH_CHECK(r_.is_privateuseone(), "hspmm: expected 'out' to be Zoom, but got CPU");
  TORCH_CHECK(dense.is_privateuseone(), "hspmm: expected 'mat2' to be Zoom, but got CPU");

  TORCH_CHECK(zoom::check_device({r_, sparse_, dense}));
  ...
  Tensor indices = at::empty({1, nnz}, at::device(at::kPrivateUse1).dtype(kLong));
  ...
  s_addmm_out_sparse_dense_zoom(values, values, newSparse, dense, 0, /*alpha*/ 1);
  get_sparse_impl(r_)->set_indices_and_values_unsafe(indices, values);

  return r_;
}

SparseTensor hspmm_sparse_zoom(const SparseTensor& sparse, const Tensor& dense) {
  SparseTensor r = at::empty({0}, sparse.options());
  hspmm_out_sparse_zoom(sparse, dense, r);
  return r;
}
...
Tensor& add_out_dense_sparse_zoom(Tensor& r_, const Tensor& dense, const SparseTensor& sparse, const at::Scalar& value) {
  TORCH_CHECK(dense.is_privateuseone(), "add: expected 'self' to be a Zoom tensor, but got a CPU tensor");
  TORCH_CHECK(sparse.is_privateuseone(), "add: expected 'other' to be a Zoom tensor, but got a CPU tensor");
  TORCH_CHECK(r_.is_privateuseone(), "add: expected 'out' to be a Zoom tensor, but got a CPU tensor");

  TORCH_CHECK(zoom::check_device({sparse, r_, dense}));
  ...
  if (sparse.is_coalesced()) {
    ...
    const dim3 block = zoom::getApplyBlock();
    dim3 grid;
    c10::DeviceIndex curDevice = -1;
    c10::zoom::GetDevice(&curDevice);
    hipStream_t stream = c10::zoom::getCurrentZoomStream(curDevice);
    if (sparse.dense_dim() == 0) {
      TORCH_CHECK(zoom::getApplyGrid(nnz, grid, curDevice), "add: Argument #0: tensor too large or too many dimensions");

      AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
        at::ScalarType::ComplexHalf, at::ScalarType::Bool, at::ScalarType::Half, at::ScalarType::BFloat16,
        commonDtype, "add_out_dense_sparse_zoom", [&] {
      ...
      TORCH_CHECK(zoom::getApplyGrid(nnz * block.x, grid, curDevice), "add: Argument #0: tensor too large or too many dimensions");

      // sparseElementwiseKernel needs values to be contiguous too
      values = values.contiguous();

      AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
        at::ScalarType::ComplexHalf, at::ScalarType::Bool, at::ScalarType::Half, at::ScalarType::BFloat16, commonDtype, "add_out_dense_sparse_zoom", [&] {
...
Tensor& add_out_dense_sparse_zoom(Tensor& r, const Tensor& dense, const SparseTensor& sparse_, const Scalar& value);

SparseTensor& add_out_sparse_zoom(const SparseTensor& t, const SparseTensor& src, const Scalar& value, SparseTensor& r_) {
  if (!t.is_sparse()) {
    return add_out_dense_sparse_zoom(r_, t, src, value);
  }

  // TODO: This test seems a bit goofy
  TORCH_CHECK(src.is_sparse(), "add(sparse, dense) is not supported. Use add(dense, sparse) instead.");

  TORCH_CHECK(t.is_privateuseone(), "add: expected 'self' to be Zoom, but got CPU");
  TORCH_CHECK(src.is_privateuseone(), "add: expected 'other' to be Zoom, but got CPU");
  TORCH_CHECK(r_.is_privateuseone(), "add: expected 'out' to be Zoom, but got CPU");

  TORCH_CHECK(zoom::check_device({r_, t, src}));
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
    at::ScalarType::Half, at::ScalarType::BFloat16, commonDtype, "add_out_sparse_zoom", [&] {
...
SparseTensor& mul_out_sparse_zoom(const Tensor& t_, const Tensor& src_, SparseTensor& r_) {
  TORCH_CHECK(r_.is_privateuseone(), "mul: expected 'out' to be Zoom, but got CPU");
  ...
  TORCH_CHECK(t_.is_privateuseone(), "mul: expected 'self' to be Zoom, but got CPU");
  TORCH_CHECK(src_.is_privateuseone(), "mul: expected 'other' to be Zoom, but got CPU");
  TORCH_CHECK(zoom::check_device({r_, t_, src_}));
...
__global__ void _sparse_sum_backward_zoom_kernel(
...
Tensor _sparse_sum_backward_zoom(const Tensor& grad_, const SparseTensor& input_, IntArrayRef dims_to_sum) {
  TORCH_CHECK(grad_.is_privateuseone(), "_sparse_sum_backward_zoom: expected 'grad_' to be Zoom tensor, but got CPU tensor");
  TORCH_CHECK(input_.is_privateuseone(), "_sparse_sum_backward_zoom: expected 'input_' to be Zoom tensor, but got CPU tensor");
  ...
  if (sum_all_sparse_dim) {
    TORCH_CHECK(!grad_.is_sparse(), "_sparse_sum_backward_zoom: expected grad Tensor to be dense since all sparse dims are summed");
    ...
    TORCH_CHECK(grad_.is_sparse(), "_sparse_sum_backward_zoom: expected grad_ Tensor to be sparse, but got dense");
    ...
    if (!sum_sparse_dim) {
      ...
      AT_ASSERT(grad_input_values.is_privateuseone());
      ...
      // config to run zoom kernel
      int64_t total_threads = input_nnz;
      const dim3 block = dim3(::min(static_cast<int64_t>(zoom::getApplyBlock().x), total_threads));
      dim3 grid;
      TORCH_CHECK(zoom::getApplyGrid(total_threads, grid, curDevice), "_sparse_sum_backward_zoom: input too large or too many dimensions");
      ...
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(kHalf, grad_values.scalar_type(), "_sparse_sum_backward_zoom", [&] {
       ...
       hipLaunchKernelGGL(( _sparse_sum_backward_zoom_kernel<scalar_t>), dim3(grid), dim3(block), 0, stream, 
          total_threads,
          grad_indices_ti,
          input_indices_ti,
...
Tensor bmm_sparse_zoom(const SparseTensor& self, const Tensor& mat2) {
  Tensor result = at::empty({self.size(0), mat2.size(2), self.size(1)}, mat2.options(), at::MemoryFormat::Contiguous);
  return bmm_out_sparse_zoom(self, mat2, result);
}

__global__ void search_end_matrix_indices_zoom_kernel(
...
void search_end_matrix_indices(int64_t* mat_el_end_indices, int64_t num_matrices, const Tensor& indices_1D) {
 ...
 hipLaunchKernelGGL(( search_end_matrix_indices_zoom_kernel), dim3(grid_size), dim3(block_size), 0, stream, 
    mat_el_end_indices,
    num_matrices,
    indices_1D_ti,
    num_elements
...
hipDataType getTensorHIPDataType(Tensor self) {
  hipDataType hip_data_type;
  switch (self.scalar_type()) {
    case ScalarType::Float:
      hip_data_type = HIP_R_32F;
      break;
    case ScalarType::Double:
      hip_data_type = HIP_R_64F;
      break;
    default:
      TORCH_CHECK(false, "Tensor types must be either float32 or float64");
      break;
  }
  return hip_data_type;
}

Tensor& bmm_out_sparse_zoom(const SparseTensor& self, const Tensor& mat2, Tensor& result) {
  ...
  auto hipsparse_handle = at::zoom::getCurrentHIPSparseHandle();
  ...
  hipsparseSpMMAlg_t mm_alg = deterministic ? HIPSPARSE_SPMM_COO_ALG2 : HIPSPARSE_SPMM_COO_ALG1;

  // Iterate through each set of 2D matrices within the 3D
  // tensor inputs, performing a matrix multiply with each
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
    values.scalar_type(), "bmm_sparse_zoom", [&] {
      ...
      for (
        ...
        if (mat_el_end_idx != -1) {
          ...
          hipDataType hip_data_type = getTensorHIPDataType(mat2_contig);
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseCreateCoo(
            &sparse_descr,
            dim_i,
            dim_j,
            sparse_nnz,
          ...
          TORCH_HIPSPARSE_CHECK(hipsparseDestroySpMat(sparse_descr));
          TORCH_HIPSPARSE_CHECK(hipsparseDestroyDnMat(dense_descr));
          TORCH_HIPSPARSE_CHECK(hipsparseDestroyDnMat(result_descr));
  ...
  // Need to transpose the result matrices since hipsparse stores
