...
*/
Tensor& sparse_sampled_addmm_out_sparse_csr_zoom(
    const Tensor& self,
    const Tensor& mat1,
    const Tensor& mat2,
    const Scalar& beta,
  ...
  sparse::impl::zoom::sampled_addmm_out_sparse_csr(mat1, mat2, beta, alpha, result);
  return result;
}

Tensor sparse_sampled_addmm_sparse_csr_zoom(
    const Tensor& self,
    const Tensor& mat1,
    const Tensor& mat2,
    const Scalar& beta,
  ...
  at::native::sparse_sampled_addmm_out_sparse_csr_zoom(self, mat1, mat2, beta, alpha, result);
  return result;
}

// result = beta * self + alpha * (mat1 @ mat2)
Tensor& addmm_out_sparse_compressed_zoom(
    const Tensor& self,
    const Tensor& mat1,
    const Tensor& mat2,
    const Scalar& beta,
    const Scalar& alpha,
    Tensor& result) {
  sparse::impl::_check_is_zoom(self, "self");
  sparse::impl::_check_is_zoom(mat1, "mat1");
  sparse::impl::_check_is_zoom(mat2, "mat2");
  sparse::impl::_check_is_zoom(result, "result");
  ...
  // From addmm_out_zoom_impl at ATen/native/zoom/Blas.cpp
  ...
  sparse::impl::zoom::addmm_out_sparse_csr(*self_, mat1, mat2, beta, alpha, result);
  return result;
}

Tensor& baddbmm_out_sparse_csr_zoom(
    const Tensor& self,
    const Tensor& mat1,
    const Tensor& mat2,
    const Scalar& beta,
  ...
  sparse::impl::zoom::addmm_out_sparse_csr(self, mat1, mat2, beta, alpha, result);
  return result;
}

Tensor& bmm_out_sparse_csr_zoom(
    const Tensor& mat1,
    const Tensor& mat2,
    Tensor& result) {
  Scalar beta(0.0);
  Scalar alpha(1.0);
  return at::native::baddbmm_out_sparse_csr_zoom(result, mat1, mat2, beta, alpha, result);
}

Tensor& addmv_out_sparse_compressed_zoom(
    const Tensor& self,
    const Tensor& mat,
    const Tensor& vec,
    const Scalar& beta,
  ...
  if (mat.layout() == kSparseCsc) {
    return addmv_out_sparse_compressed_zoom(self, mat.to_sparse_csr(), vec,
        beta, alpha, result);
  }
  TORCH_CHECK(mat.layout() != kSparseBsc, "addmm_out_sparse_csr_zoom currently does not support layout SparseBsc for input mat.");

  TORCH_CHECK(mat.dim() == 2, "addmv: Expected mat to be 2-D");
  TORCH_CHECK(vec.dim() == 1, "addmv: Expected vec to be 1-D");

  // Preprocessing code is copied from TORCH_IMPL_FUNC(addmv_out_zoom) at
  // aten/src/ATen/native/zoom/Blas.cpp
  ...
  sparse::impl::zoom::addmv_out_sparse_csr(mat, vec, beta, alpha, result);
  return result;
}
...
*/
std::tuple<Tensor&, Tensor&> triangular_solve_out_sparse_csr_zoom(
    const Tensor& B,
    const Tensor& A,
    bool upper,
    bool transpose,
    ...
    Tensor& X,
    Tensor& clone_A) {
  sparse::impl::zoom::triangular_solve_out_sparse_csr(A, B, X, upper, transpose, unitriangular);
