...
hipsparseStatus_t destroyConstDnMat(const hipsparseDnMatDescr* dnMatDescr) {
  return hipsparseDestroyDnMat(const_cast<hipsparseDnMatDescr*>(dnMatDescr));
}
...
void check_supported_hip_type(hipDataType hip_type) {
  if (hip_type == HIP_R_16F) {
    hipDeviceProp_t* prop = at::zoom::getCurrentDeviceProperties();
    TORCH_CHECK(
        prop->major >= 5 && ((10 * prop->major + prop->minor) >= 53),
        "Sparse operations with Zoom tensors of Float16 type are not supported on GPUs with compute capability < 5.3 (current: ",
        prop->major,
        ".",
        prop->minor,
        ")");
...
hipsparseIndexType_t getHipSparseIndexType(const c10::ScalarType& scalar_type) {
  if (scalar_type == c10::ScalarType::Int) {
    ...
    TORCH_INTERNAL_ASSERT(
        false, "Cannot convert type ", scalar_type, " to hipsparseIndexType.");
  }
}
...
hipsparseDnMatDescr_t createRawDnMatDescriptor(const Tensor& input, int64_t batch_offset, bool is_const=false) {
  ...
  auto order = HIPSPARSE_ORDER_COLUMN;
  ...
  hipDataType value_type = ScalarTypeToHIPDataType(input.scalar_type());
  check_supported_hip_type(value_type);

  // NOTE: Ideally, in the const case, we would use hipsparseConstDnMatDescr_t
  // and hipsparseCreateConstDnMat, but those were introduced in CUDA 12, and we
  // still need to support CUDA 11
  hipsparseDnMatDescr_t raw_descriptor;
  ...
  if (ndim >= 3 && batch_offset == -1) {
    int batch_count =
        at::native::zoom_int_cast(at::native::batchCount(input), "batch_count");
    TORCH_HIPSPARSE_CHECK(hipsparseDnMatSetStridedBatch(
        raw_descriptor, batch_count, input_strides[ndim - 3]));
  }
  return raw_descriptor;
}

HipSparseDnMatDescriptor::HipSparseDnMatDescriptor(const Tensor& input, int64_t batch_offset) {
  descriptor_.reset(createRawDnMatDescriptor(input, batch_offset));
}

HipSparseConstDnMatDescriptor::HipSparseConstDnMatDescriptor(const Tensor& input, int64_t batch_offset) {
  descriptor_.reset(createRawDnMatDescriptor(input, batch_offset, /*is_const*/true));
}
#endif // AT_USE_CUSPARSE_GENERIC_API() || AT_USE_HIPSPARSE_GENERIC_API()

HipSparseDnVecDescriptor::HipSparseDnVecDescriptor(const Tensor& input) {
  ...
  hipsparseDnVecDescr_t raw_descriptor;
  TORCH_HIPSPARSE_CHECK(hipsparseCreateDnVec(
      &raw_descriptor, input.numel(), input.data_ptr(), value_type));
  descriptor_.reset(raw_descriptor);
}

HipSparseSpMatCsrDescriptor::HipSparseSpMatCsrDescriptor(const Tensor& input, int64_t batch_offset) {
  ...
  hipsparseIndexType_t index_type =
      getHipSparseIndexType(crow_indices.scalar_type());
  hipDataType value_type = ScalarTypeToHIPDataType(input.scalar_type());
  check_supported_hip_type(value_type);
  ...
  hipsparseSpMatDescr_t raw_descriptor;
  ...
  if (ndim == 3 && batch_offset == -1) {
    ...
    if (crow_indices.dim() >= 2 || values.dim() >= 2 ||
      ...
      TORCH_HIPSPARSE_CHECK(hipsparseCsrSetStridedBatch(
          raw_descriptor,
          batch_count,
          crow_indices.stride(-2),
          values_->stride(-2)));
